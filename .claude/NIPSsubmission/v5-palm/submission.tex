...
The step is conformally symplectic: $\det J=(1-\gamma)^d$, and $(1-\gamma\phi(q'))^d$ when position-gated. Checkpoints (dim 4, hidden 64, $\varepsilon=0.05$, 150 epochs) are \emph{designed} ($SO(2)$-invariant $V_\theta$, 5 seeds) or \emph{emergent} (MLP $V_\theta$ on identical symmetric data, 3 seeds). On a trained designed-$SO(2)$ vacuum the massive radial mode's $n_{1/2}(\gamma)$ is a V minimized at $\gamma_{\rm crit}=2\varepsilon\mu_{\rm rad}$. The overdamped branch follows the $\mu^{-2}$ law up to $\varepsilon\mu\approx\gamma/2$ and then saturates at a mass-independent floor (log-slopes $-1.006$ and $+1.23$--$+1.27$; 5 seeds, verification, Appendix~\ref{app:budget}). The flat coset (Hessian $\mu^2\approx10^{-15}$) is the \emph{same curve at $\mu\to0$}, where $\gamma_{\rm crit}\to0$: latch, overdamped register and underdamped working memory are three regimes of one curve, evaluated at two values of $\mu$ --- not two laws. The law generalizes to emergent units (evidence). On three emergent MLP checkpoints trained on the same symmetric data the coset-tracked $T=0$ half-life traces the same V-curve: argmin $0.902\pm0.003\times\gamma_{\rm crit}$, log-slopes $-1.0020\pm0.0003$ / $+1.116\pm0.011$, 3/3 seeds. Across both families the curve spans $\mu^2\in[1.7\times10^{-12},\,7\times10^{-2}]$. That low endpoint is the ring-profile probe's resolution on a checkpoint whose Hessian $\mu^2$ is machine zero rather than a spectral mass, so eleven orders is one curve on one instrument. The $\mu\to0$ corner carries its own model-side bound of $n_{1/2}\ge9.5\times10^{15}$ steps by direct $T=0$ rollout (Appendix~\ref{app:emergent}).
\begin{figure}[t]
\centering
...
\caption{The coset diffusion law, panel by panel. (a) $D_\theta\propto T$ at five frictions; (b) $D_\theta\propto(2-\gamma)/\gamma$ at five temperatures; (c) the ratio $\hat D_\theta/D_\theta^{\rm pred}$ over all $25$ cells, $1.0068\pm0.0219$ with the band drawn. \emph{Multi-seed status: one trained designed-$SO(2)$ checkpoint (seed 44), the single checkpoint on which the $25$-cell grid was run; the five-seed statements of this appendix are the sign-flip and latch entries, not this grid.} \texttt{langevin\_noise="fdt"}, Newtonian kinetic mode, $\Delta=0.5$ rad. \emph{Designed testbed: verification.}}
\label{fig:dlaw}
\end{figure}
...
\begin{table}[!htb]\centering\small
\caption{The shape claims on all four instruments ($\delta=0.05$ rad, 48 $\gamma$ values, 3 emergent seeds). The rollout rate instrument reproduces the Jacobian; both threshold instruments fail below $\gamma_{\rm crit}$, so no rollout $n_{1/2}$ is ever quoted from a threshold there.}
\begin{tabular}{lcccc}
\toprule
...
\caption{The emergent V-curve un-collapsed, and the negative beside it. (a) The $T=0$ half-life against friction per emergent seed from the one-step Jacobian, each with its own $\gamma_{\rm crit}=2\varepsilon\mu_{\rm soft}$ dotted; the designed coset has no finite half-life at any $\gamma$ and is drawn at the axis top. (b) $\big||\lambda_{\rm coset}|-1\big|$: exact to the float64 floor for the designed symmetry and short by $\sim10^{-3}$ on every emergent seed, which is the pseudo-Goldstone statement of this section in one panel. \emph{Three emergent MLP seeds plus a matched designed control: evidence.}}
