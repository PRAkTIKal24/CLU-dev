...
\item \textbf{The paid-access frame and its two failure modes (\S2--3).} Reachability splits into \emph{reach} (kinematic; target outside the causal box $C_T$, $L_i=T\varepsilon c/\sqrt{M_i}$) and \emph{escape} (energetic; inside, behind a barrier); the two are cured by different mechanisms with different receipts. \emph{[proven; theory note + Anonymous 2026.]}
...
\paragraph{Reachability, made falsifiable.} For the governed map $\Phi_{\varepsilon,\gamma}$, the $T$-step \textbf{reachable set} from $z_0$ is $R_T(z_0)=\{\Phi^n(z_0):1\le n\le T\}$, position shadow $Q_T(z_0)$. \textbf{Access} is provably enlarging $Q_T$. The theory note proves (Prop-A2) that in relativistic mode one drift advances $q$ by at most $\varepsilon c/\sqrt{M_i}$ per coordinate, so
\[
...
\emph{Reporting grade: evidence.} \S4 leaves the designed testbed for \textbf{trained} memories; each claim is scoped and priced in-line. Where the receipt says a cheaper black box wins, we say so. \textbf{The scoreboard sentence, stated up front: external benchmarks won on their own headline metric = ZERO.}
...
\emph{Reporting grade: evidence, with the substrate stated --- a \textbf{designed} store addressed through a \textbf{learned, frozen} encoder $\varphi$, not a trained CLU checkpoint. Every number in this subsection is $6$ seeds of \emph{its own} protocol, never \S3/\S4.1--4.3's $5$ seeds.}

The three pillars above ration or route \emph{within} a memory. The comparison a reviewer asks for next is against a floor handed the \emph{same} compute budget, and we report it here. In the native regime --- pixel-space corruption of a $\varphi$-addressed store, where \textbf{no mask oracle can be constructed} --- the confidence-gated anytime read \textbf{ties} the matched-compute feedforward-in-$\varphi$ floor ($\mathbf{+0.8\pm1.6}$ \textbf{pp, 6 seeds}, $3/6$ positive) and beats $1$-shot kNN-in-$\varphi$ by $+1.1\pm1.4$, while beating every mechanism control: $\mathbf{+4.6\pm2.2}$ \textbf{pp over an equal-energy random kick, $6/6$ seeds}, $+4.4$ over an ensemble of restarts, ungated retry collapsing to $0.000$, and auto-stopping at $\mathbf{1.40\pm0.20\times}$ compute. \textbf{It ties; it does not win} --- and the tie is the result, because it is the first cell we have measured in which the anytime read is not \emph{below} the correct floor. It is also the measured object behind \S5's proof-of-concept framing: the deferred matched-compute comparison, run in one regime, comes back level.

\textbf{Scope, in-sentence (C-5).} One dataset (MNIST-class images, $\varphi$ fit on the first task's classes only and frozen), one corruption level (pixel dropout $p=0.8$), one crowded store ($199$--$200$ wells, end-of-stream), $6$ seeds, laptop-CPU. At the easier level ($p=0.5$) the cell is \textbf{saturated} --- first pass $0.99$, floor $1.000$ --- and every line is flat; on CIFAR-class images the $1$-shot kNN-in-$\varphi$ floor is $1.000$ and the same read sits \emph{below} it ($-0.055\pm0.019$). A saturated floor is unbeatable, which is why the hard cell is the one that carries the result, and why nothing here generalises past it.

\paragraph{The negative that travels with any anytime claim (C-9).} Where a mask oracle \emph{can} be constructed, the verdict is a \textbf{decision-grade NO with headroom present}, so it cannot be blamed on saturation: built from ambiguity (block occlusion plus a contracted crowded store, $3$ seeds, first pass $0.26$--$0.93$ off ceiling), the gated read does \textbf{not} beat the ML-optimal masked-erasure oracle ($=1.000$) in \emph{any} of $8$ cells ($-3.5$--$-38$ pp). The corrected status is precise: the \textbf{static-retrieval instantiation} of the compute-adaptive-read claim is \textbf{closed} by that measurement, while the \textbf{mechanism} survives all of its controls (kick and ensemble flat or declining, directed boost $+2.3$--$+36.2$ pp, auto-stopping $\times1.20$--$1.81$) --- losing to an oracle that is \emph{told which coordinates were erased} is the metric-native ceiling, not a falsification of the dial. On any erasure protocol the mandatory baseline is that masked observed-dims nearest-neighbour oracle, never the na\"ive full-vector one.

\paragraph{Quoting an anytime curve, including a flat one (C-6).} A flat anytime curve does \emph{not} license ``the store carries nothing.'' Flatness licenses only the disjunction \emph{``the store carries nothing \textbf{or} it cannot be addressed,''} and the two are separable by a control we ran: same store, same physics, same frozen budget grid, the read moves $0.0223\to0.8219\to0.8711\to$ plateau under oracle addressing where the shipped read is flat at $0.0004$ --- quoted as the curve it is, never as an endpoint pair. And \textbf{no uniqueness is claimed for the anytime shape}: that figure is an occupied venue (deep-equilibrium models, energy-based transformers, recurrent-depth architectures); what is claimed is a shape-and-mechanism result measured against its own controls.

\section{Position, scope, and horizon}
\paragraph{The position, restated.} On a conservative memory, test-time compute is \emph{paid access}, and the price list is physical: a squeeze buys escape for a bounded, re-absorbable injection and \textbf{prices reach exponentially} (it exceeds the box only by its instantaneous displacement $p_0\sinh\zeta/M_0$, at energy cost $e^{2\zeta}$); a wormhole buys \textbf{unbounded} reach for a $\detJ=1$ receipt and a \emph{fixed} discrete ledger, and transports rather than destroys stored content --- where an untrained $\detJ=0$ \textbf{state-replacing} jump, which reaches by overwriting the state, provably erases it. (Deciding \emph{whether} to take that certified edge is separate: \S4.2's learned decision head routes \emph{through} the channel and inherits its receipt --- decision and transport are orthogonal.) These are theorems verified to machine precision (\S3), composing with the memory's BIBO guarantees \emph{only under coercive-component screening} --- a bounded, or even free, energy ledger is not enough (App.~B.2). Where a learned halting head or an energy verifier (Gladstone et al.\ 2025) escalates \emph{the same operation} with no structural account, a symplectic memory prices each access mechanism \emph{and certifies what it did to the state}. The honest pillars of \S4 delimit what the extra compute does \emph{on this memory}: a rationed relaxation budget buys accuracy on the memory we trained (\S4.1, \S4.3), and we offer that as an \textbf{empirical proof of concept}, not as a comparative result --- \textbf{no matched-compute floor and no matched-bytes exemplar store is run against \S4.1--4.3 anywhere in this paper}, and that comparison is named here as \textbf{future work} (the one matched-compute comparison we do report, \S4.4, comes back a \emph{tie}). They also delimit where a cheaper black box wins (routing): a certificate stack is a design discipline, not a guarantee of dominance. \textbf{The position has a third corner, and it is an impossibility result.} Beyond a single access mechanism, the same accounting binds the store's \emph{read}: with a quadratic payload channel, the $q_2(0)=0$ anti-decoration guard and a fixed read budget, a store cannot have all three of \textbf{exact value fidelity}, \textbf{amplitude-independent address hold} and \textbf{amplitude-independent read latency} --- pick two. The shipped store drops the second, which is why effective lifetime correlates $r=-0.85$ with $a_i^2$; the recommended gated-stiffness channel drops the third. \textbf{Both proposed fixes to that corner are refuted, and neither may be described as available.} The corner to lead with is the third: \emph{dropping amplitude-independent latency \textbf{is} the compute-adaptive-read dial --- a faded memory costs more integration steps to read, which is a physical, measurable statement a timestamped row cannot make.} (Proved and measured on this program's decaying-store instantiation --- a designed store with a quadratic payload channel and a fixed read budget, $3$ seeds, laptop-CPU --- not on the \S3 unit.)

\paragraph{Scope (stated, not buried).} The \S3 verifications use \textbf{oracle channel placement}, dim $2$/$4$, $5$ seeds, laptop-CPU; the \S4 learned-memory pillars are MQAR-style, vocab-$256$, kv/$N$ small, laptop-CPU. No claim here is at scale, and none uses learned placement. And the substrate scope, stated once, in our own voice: \textbf{these laws govern the store; end-to-end performance additionally depends on the encoder, measured separately, $\varphi$-bytes ledgered.}
...
\item \textbf{Mix; do not rely on the squeeze alone.} The squeeze family is a one-parameter subgroup whose orbit is a single hyperbola, hence \textbf{reducible/non-ergodic}; read it as a mass-metric-preconditioned \emph{reach} move and use $\tfrac12\,\mathrm{MALA}(\sigma^\star)+\tfrac12$ sign-symmetrized squeeze-MH (a mixture of $\pi$-reversible kernels is $\pi$-reversible). \textbf{Measured, on a designed store (evidence; MNIST, designed/closed-form on every line, an $8$-cell ladder at seed $0$ re-measured across $5$ seeds \emph{of that protocol} --- never \S3/\S4's $5$ seeds):} the lift is the \textbf{directed} symplectic re-launch: equal-energy random kicks and ensembles of $k$ independent restarts are \textbf{dead flat in all 8 cells}, while confidence-gated CLU retry rises $\mathbf{+6.6}$--$\mathbf{+76.2}$ \textbf{pp} and \textbf{auto-stops at $\times1.2$--$1.8$} compute; the confidence gate is load-bearing --- ungated retry-all collapses $0.96\to0.004$ at $9\times$ compute. Three companions travel with that sentence. \emph{(i) The floor:} \textbf{CLU draws a rising, auto-stopping accuracy-vs-compute curve that a saturated feedforward memory structurally cannot draw, while remaining below that saturated floor on masked-pixel retrieval} --- on $5$ seeds of the protocol the gap to that floor is $-3.9$--$-20.7$ pp under masking and $-9.7$--$-48.2$ pp under noise, negative in $40/40$ (seed, cell) pairs, with gated lift $+0$--$+79.7$ pp and $0/40$ violations by either mechanism control. \emph{(ii) The protocol split:} the boost recovers mask queries strongly ($+36$--$+76$ pp) and Gaussian-noise queries weakly ($+6.6$--$+34.8$ pp), and \textbf{nothing recovers past $\sigma\approx0.4$}. \emph{(iii) The acceptance discipline:} cosine-to-nearest-well is a \emph{ranking}, never an acceptance, signal. \textbf{And the same decision-grade NO with headroom present that \S4.4 states applies here}: against the ML-optimal masked-erasure oracle ($=1.000$) the gated read does \textbf{not} win in any of $8$ cells ($-3.5$--$-38$ pp, $3$ seeds, first pass $0.26$--$0.93$ off ceiling), so this is a mechanism claim and never a leaderboard claim.
...
experiment commit & \texttt{6f2384c} (branch tip \texttt{agent/experiment-engineer/paid-access-experiments}, off \texttt{main}@\texttt{63fea62}; not pushed) \\
JAX & $0.9.0$ (main venv reused, protocol \S4) \\
kinetic modes & relativistic (reach, wormhole, throat); \texttt{newtonian\_learned} (Newtonian control, injection cert) \\
mass band (prerequisite) & $[4.0, 0.25]\Rightarrow M_{\rm eff,0}=4.0$ (heavy reach coord), $16\times$ contrast $\Rightarrow S^{(M)}$ directional \\
$c$ / rest mass / $m_0$ & $1.0$ / $1.0$ / $1.0$ \\
dt / $T$ (reach horizon) & $0.05$ / $100\Rightarrow L=T\varepsilon c/\sqrt{M_0}=2.5$, $v_{\max,0}=0.5$ \\
...
init & $q_0=0$, $p_0=(1.2,0,\dots)+0.02\,\mathcal N$ (KE$_0<\Delta V_b\Rightarrow$ plain relax escape-blocked by design) \\
landing criterion & $\min_{\rm traj}|q_0-d|<0.4$ \\
seeds & reach $\{0,1,2,3,4\}$; latch/injection \texttt{default\_rng(0)} \\
dims & $2$ (headline) and $4$ (identical result) \\
\bottomrule
\end{tabular}}

\subsection*{A.2\quad \S4.1 calibrated gate (\texttt{v1-pivot}; memory-agnostic transfer from \texttt{minus-the-physics} Part~B)}
The gate/calibration/LTT numbers are from \texttt{v1-pivot}. The \textbf{memory-agnostic} finding (CM-2 approved wording) --- the identical stack on a modern Hopfield memory reproduces the calibration jump, raw $0.18\to0.88$ vs CLU $0.43\to0.87$ --- is from \texttt{minus-the-physics} Part~B ($5$ seeds), whose provenance is inherited here.
...
flag & value \\
\midrule
commit & \texttt{572c708} (branch \texttt{agent/experiment-engineer/v1-pivot}, off \texttt{main}@\texttt{dbeb2c2}; not pushed) \\
Hopfield-transfer source & \texttt{minus-the-physics} Part~B, $5$ seeds (same MQAR vocab-$256$, kv$\le32$ probes; CM-2) \\
task & MQAR CLU-EBM, vocab $256$, kv$\in\{16,24,32\}$, $128$--$144$ trials$\cdot$seed$^{-1}\cdot$level$^{-1}$ \\
seeds & $5$ (base $42$); $90$ models, $609$\,s CPU \\
write & kinetic relativistic, potential mlp, hidden $128$, epochs $400$, lr $1\mathrm{e}{-3}$, batch $16$, k\_steps $50$, buffer $128$, friction $0.3$, temperature $0.3$, input noise $\sigma$ $0.05$, PCD \\
gate & calib features \texttt{r\_margin}, learned $\tau$ at write time, $p_{\rm exit}=0.5$; cost checkpoints $300/1200/2100/3000$ \\
...
\texttt{langevin\_noise} & legacy (default); lyapunov N/A (PCD write, no MSE/Lyapunov) \\
\bottomrule
\end{tabular}}

\subsection*{A.3\quad \S4.2 routing (\texttt{v1-router-baseline}, \texttt{v1-wormhole-routing})}
\noindent{\footnotesize
\begin{tabular}{@{}p{0.24\textwidth}p{0.70\textwidth}@{}}
\toprule
flag & value \\
\midrule
commit & \texttt{52330f8} on \texttt{9339a13}, off \texttt{main}@\texttt{9a13455} (branch \texttt{agent/experiment-engineer/v1-router-baseline}; not pushed) \\
only non-default flag & \texttt{experiment\_v1\_wormhole.n\_seeds=5} (default $2$) \\
lattice/task & $N\in\{4,8\}$, embed\_dim $12$, embed\_scale $2.0$, vocab $128$, kv\_per\_unit $3$, query\_cue\_noise $0.05$ \\
write & kinetic relativistic, potential mlp (coercive), hidden $128$, epochs $400$, lr $1\mathrm{e}{-3}$, batch $16$, k\_steps $50$, buffer $128$, friction $0.3$, temperature $0.3$ \\
retrieval/routing & dt $0.05$, relax\_steps/route\_steps $250$, governor\_sensitivity $0.95$, $\kappa_{\rm wh}=\kappa_{\rm chain}=2.0$, gate\_z\_threshold $0.0$, gate\_z\_width $0.7$, gate\_route\_threshold $0.5$ \\
router MLP & hidden $32$ (\textbf{449 params}), epochs $300$, lr $3\mathrm{e}{-3}$, l2 $1\mathrm{e}{-3}$ \\
FLOPs model & flops\_grad\_factor $6.0$, flops\_verlet\_grads $2.0$; routed leg $=2$ units (flat in $N$), chain $=N$ units \\
workload mixes & $[[.5,.5],[.8,.2],[.95,.05]]$ \\
\bottomrule
\end{tabular}}

\subsection*{A.4\quad \S4.3 regime map (\texttt{v1-hopfield-stress}, \texttt{regime-remap-2000ep}, \texttt{anchor-robustness} item~2)}
\noindent{\footnotesize
\begin{tabular}{@{}p{0.24\textwidth}p{0.70\textwidth}@{}}
\toprule
flag & value \\
\midrule
commit & \texttt{63fea62} (main, w6-integrated); analysis read-only, artifacts under \texttt{.claude/} \\
experiment & \texttt{experiment\_v1\_gate} regime map; dtype f32 \\
\textbf{train\_epochs} & $\{500,1000,2000,4000\}$ (the arm) \\
kinetic / potential & relativistic / mlp (coercive) \\
embed\_dim / hidden / vocab & $16$ (CLU dim $32$) / $128$ / $256$ ($512$ for kv$128$) \\
retrieval & relax\_steps $300$, governor\_sensitivity $0.95$, calib \texttt{r\_margin}, $p_{\rm exit}=0.5$, cost ladder $300/1200/2100/3000$ \\
Hopfield & $\beta\in\{2,5,20\}$; iteration sweep $\{1,2,3,5,10\}$ (Item~3) \\
\texttt{langevin\_noise} & legacy (FDT-violating default; kept for baseline continuity) \\
seeds & $\{42\text{--}46\}$ (Item~1) / $\{42,43,44\}$ (Item~2) / $\{42,43\}$ (anchor item~2, $2$ episodes/cell) \\
\bottomrule
\end{tabular}}

\subsection*{A.5\quad \S3.2.1 latch payoff $+$ Appendix~B.2 BIBO battery (\texttt{v1-certificate-payoff})}
These runs extend the \S3 battery with two new arms; the defaults preserve the \S3.1/\S3.2/C.1 numbers \textbf{bit-identically} (reach table, latch transit, and squeeze injection all reproduce \texttt{paid-access-experiments} to the digit).
...
flag & value \\
\midrule
commit & \texttt{27f232f} (branch tip \texttt{agent/experiment-engineer/v1-certificate-payoff}; payoff code \texttt{d9a9f38}, config \texttt{f2a85aa}); base local \texttt{main}@\texttt{37dc664}; not pushed \\
JAX & $0.9.0$ (main venv reused, \texttt{-{}-no-sync}; protocol \S4) \\
training & \textbf{none} --- analytic potentials, oracle channel placement (learned entrance-steering out of scope) \\
kinetic mode & relativistic (reach, wormhole, throat, BIBO); \texttt{newtonian\_learned} (Newtonian control, injection cert) \\
mass band (prerequisite) & $[4.0,0.25]\Rightarrow M_{\rm eff,0}=4.0$, $16\times$ contrast \\
$c$ / $m_0$ / dt & $1.0$ / $1.0\Rightarrow v_{\max,0}=0.5$ / $0.05$ \\
$\gamma$ & reach: $0$ (sharp box $L=2.5$); \textbf{BIBO: $0.02$} (bounded arms must settle) \\
BIBO potential & $k=1.0$, $\epsilon=0.02\Rightarrow x_b=3.5355$, $V_b=3.125$; transverse conf $=4.0$ \\
...
seeds & reach/BIBO $\{0,1,2,3,4\}$; latch cloud \texttt{default\_rng(0)}; robustness seed$_0\in\{0,7\}$ \\
dims & $2$ (headline) and $4$ (identical structure) \\
tests & \texttt{pytest tests/test\_paid\_access.py tests/test\_core.py} $\to$ $16$ passed ($7$ paid-access incl.\ $2$ new regression tests $+$ $9$ core) \\
\bottomrule
\end{tabular}}

\paragraph{Cross-section reproducibility note (C-7).} The two configuration axes a reviewer must track are \textbf{damping} and \textbf{training epochs}. \emph{Damping:} \S4.1 uses $\gamma$/friction $0.3$ at write, PCD; \S3's reach rollout uses $\gamma=0$ for a sharp box, while Appendix~B.2's BIBO battery uses $\gamma=0.02$ \textbf{because a bounded arm must be able to settle} (at $\gamma=0$ a conservative orbit never converges, so ``bounded'' would be untestable); \S4.3 uses \texttt{langevin\_noise=legacy}. \emph{Epochs (the axis that governs the \S4.1$\leftrightarrow$\S4.3 comparison):} \textbf{\S4.1 trains $400$ ep; \S4.3 sweeps $\{500,1000,2000,4000\}$ ep.} These are different budgets, and the paper states so at the point of collision: \S4.1's escalatable-accuracy headroom is a $400$-ep (not-yet-converged) property, and \S4.3, App.~C.4.a gives the converged ($2000$-ep) counterpart where the gate's accuracy \emph{matches} full-budget and the payoff is rationing only. The two are reconciled inline in \S4.1 point~2, not asserted away here.
...
\subsection*{C.1\quad Reach battery, full grid (Table~2; \texttt{paid-access-experiments} \S7.1, dim $2$, $5$ seeds, $L=2.5$)}
\noindent{\footnotesize
\begin{tabular}{@{}lcccccc@{}}
\toprule
arm & $d{=}0.8$ & $1.6$ & $2.4$ & $3.2$ & $4.0$ & $5.0$ \\
\midrule
\texttt{plain\_relax} & 0 & 0 & 0 & 0 & 0 & 0 \\
squeeze $S^{(M)}$ & 1 & 1 & 1 & 1 & \textbf{0}$^\dagger$ & \textbf{0}$^\dagger$ \\
wormhole & 1 & 1 & 1 & 1 & 1 & 1 \\
\texttt{newtonian\_squeeze} (control) & 1 & 1 & 1 & 1 & 1 & 1 \\
state-replacing map (\texttt{no\_physics\_router}) & 1 & 1 & 1 & 1 & 1 & 1 \\
throat/dense-$V$ & 1 & 0.8 & 0 & 0 & 0 & 0 \\
\bottomrule
\end{tabular}}

\noindent{\footnotesize $^\dagger$ \textbf{The squeeze's $0$ at $d\in\{4.0,5.0\}$ is ``priced out of the swept $\zeta\le2.0$ grid,'' not ``cannot reach'' (the pricing law, \S3.2).} The reachable radius is the bracket $[L,\,L+p_0\sinh\zeta/M_0]$ with $L=2.5$, $p_0=1.2$, $M_0=4.0$; the \emph{same} verified bracket predicts $d=4.0$ lands at $\zeta\ge2.0105$ (energy $\approx e^{4.02}H$) and $d=5.0$ at $\zeta\ge2.6441$ (energy $\approx e^{5.29}H$) --- reach beyond $L$ is bought at exponentially rising energy, whereas the wormhole reaches all $d$ at a fixed ledger. These two $\zeta$ values are analytic predictions from the verified bracket formula, not new measurements; the swept grid ended at $\zeta=2.0$.}

\paragraph{Note on the two ``router'' objects.} The \texttt{no\_physics\_router} arm above is an \emph{untrained analytic constant map} $(q,p)\mapsto(b,p)$ (\texttt{training: none}, A.5) --- a \textbf{state-replacing} baseline, distinct in kind from \S4.2's \texttt{router\_mlp}, a $449$-param \emph{learned decision head} that routes \emph{through} the wormhole's $\detJ=1$ edge. Only the former has $\detJ=0$; the latter inherits the wormhole receipt.

\noindent Certificates on every arm: wormhole $\detJ=[1.0]\times6$ (exact), ledger err $[0.0]\times6$; \textbf{state-replacing map $\detJ=[0.0]\times6$ (measured by forward-mode autodiff --- the map $(q,p)\mapsto(b,p)$ has Jacobian $\mathrm{blockdiag}(0_d,I_d)$, so it is volume-annihilating and non-invertible, not merely uncertified)}; squeeze $\det S^{(M)}=1.000\ (\pm4\mathrm{e}{-6})$, symplectic err $6\mathrm{e}{-8}$--$1.3\mathrm{e}{-6}$, $H$-ratio $\le e^{2\zeta}$ (matched quadratic $H$); forbidden state-dependent gate $\detJ=2.05$ (design guard). Latch transit: zero-shift $\Delta Q=0.0$; across-coset $\Delta Q=0.2500=p^\top X\Delta$; squeeze $\Delta Q=1.2\mathrm{e}{-7}$; random shift erases ($\detJ=1$ notwithstanding).

\paragraph{C.1.b --- The latch-payoff cloud (\S3.2.1), all arms.} $16$ incoming states, capture ball radius $0.3<\rho=0.35$, $\mathrm{std}(Q_{\rm in})=0.0803$; per-arm reconstruction error where defined.

\noindent{\scriptsize\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}lccccc@{}}
\toprule
arm & $\detJ$ & $\mathrm{std}(Q_{\rm out})$ & $\mathrm{std}(Q_{\rm out})/\mathrm{std}(Q_{\rm in})$ & $\Delta Q$ std & reconstruction err \\
\midrule
...
\textbf{state-replacing map} (\texttt{no\_physics\_router}) & $\mathbf{0.0}$ & $\mathbf{0.0}$ & $\mathbf{0.0}$ & $0.0803$ & \textbf{undefined (non-invertible)} \\
\bottomrule
\end{tabular}}

\noindent Replication (dim $\times$ seed$_0$): $\mathrm{std}(Q_{\rm out})_{\rm wormhole}=\mathrm{std}(Q_{\rm in})$ in every cell --- $0.0803$ (dim $2$, s0), $0.0679$, $0.0533$, $0.0448$ --- and $\mathrm{std}(Q_{\rm out})_{\rm router}=0.00\mathrm{e}{+00}$ in every cell.

\subsection*{C.2\quad Routing, full grid (\texttt{v1-router-baseline}, $5$ seeds; acc local/distant, FLOP/query)}
\noindent{\footnotesize
\begin{tabular}{@{}lcccc@{}}
\toprule
arm & $N{=}4$ acc (L/D) & $N{=}4$ FLOP & $N{=}8$ acc (L/D) & $N{=}8$ FLOP \\
\midrule
\texttt{local\_only} & $0.500$ ($1.00$/$0.00$) & $5.88\mathrm{e}7$ & $0.500$ ($1.00$/$0.00$) & $5.88\mathrm{e}7$ \\
gated (energy) & $0.887\pm0.139$ ($0.90$/$0.88$) & $1.18\mathrm{e}8$ & $0.715\pm0.172$ ($0.82$/$0.61$) & $1.18\mathrm{e}8$ \\
dense & $0.500$ ($0.00$/$1.00$) & $1.18\mathrm{e}8$ & $0.448\pm0.070$ ($0.00$/$0.90$) & $1.18\mathrm{e}8$ \\
chain & $0.652\pm0.159$ ($0.90$/$0.41$) & $1.76\mathrm{e}8$ & $0.548\pm0.138$ ($0.82$/$0.28$) & $\mathbf{2.94\mathrm{e}8}$ \\
calibrated (energy head) & $0.860\pm0.122$ ($0.72$/$1.00$) & $1.34\mathrm{e}8$ & $0.677\pm0.174$ ($0.46$/$0.90$) & $1.49\mathrm{e}8$ \\
\textbf{\texttt{router\_mlp} (no physics)} & $\mathbf{1.000\pm0.000}$ ($1.00$/$1.00$) & $\mathbf{8.81\mathrm{e}7}$ & $\mathbf{0.948\pm0.070}$ ($1.00$/$0.90$) & $\mathbf{8.81\mathrm{e}7}$ \\
\bottomrule
\end{tabular}}

\noindent Per-mix: the router leads on accuracy AND FLOPs at every mix $\{50/50, 80/20, 95/5\}$ and both $N$. Salvaged mechanism: $1$-hop routed leg flat $1.18\mathrm{e}8$; chain $1.76\mathrm{e}8\to2.94\mathrm{e}8$ with distant recall $0.41\to0.28$.

\subsection*{C.3\quad Regime map --- Hopfield iteration parity (\texttt{regime-remap-2000ep} Item~3, $3$ seeds)}
\noindent{\footnotesize
\begin{tabular}{@{}lcccccc@{}}
\toprule
...
\noindent Hopfield reaches its ceiling in a single matvec at $\beta\ge5$; $10\times$ more iterations change accuracy by $\le0.003$. Cost floor $O(\text{kv}\cdot d)$.

\subsection*{C.4\quad Regime map --- settled full grid (\texttt{regime-remap-2000ep}, $198$ jobs, $0$ failures)}

\paragraph{C.4.a --- Capacity axis, paired $500$ vs $2000$ ep (corr $=0$; $n=8$ pooled seeds; Figure~\ref{fig:regime}a--b).} $\Delta=$ gate $-$ Hopfield; savings $=$ intra-CLU (full-budget CLU cost / gated CLU cost).

\noindent{\footnotesize\setlength{\tabcolsep}{3pt}
...
\paragraph{C.4.b --- Epoch-scaling frontier $\{500,1000,2000,4000\}\times$ kv$\{32,64,96,128\}$ ($n=3$; \texttt{fig\_frontier\_clean.png}, plotted as Figure~\ref{fig:frontier} in \S4.3).} Hopfield band across these cells $=0.947$--$0.976$ (epoch-independent, $1$ matvec).
...
kv32 & gate acc & $0.30\pm0.04$ & $0.98\pm0.02$ & $\mathbf{1.00\pm0.01}$ & $0.89\pm0.05$ & \textbf{2000 ($+0.02$)} \\
kv64 (N256,v256) & fidelity & $0.41\pm0.02$ & $0.82\pm0.04$ & $1.00\pm0.00$ & $1.00\pm0.00$ & \\
kv64 & gate acc & $0.06\pm0.02$ & $0.42\pm0.02$ & $\mathbf{0.99\pm0.01}$ & $0.97\pm0.01$ & \textbf{2000 ($+0.02$)} \\
kv96 (N384,v256) & fidelity & $0.39\pm0.02$ & $0.24\pm0.06$ & $0.97\pm0.01$ & $1.00\pm0.00$ & \\
kv96 & gate acc & $0.02\pm0.00$ & $0.11\pm0.01$ & $0.92\pm0.02$ & $\mathbf{0.975\pm0.00}$ & \textbf{4000 ($+0.03$)} \\
kv128 (N384,v512) & fidelity & $0.40\pm0.02$ & $0.09\pm0.01$ & $0.71\pm0.06$ & $1.00\pm0.00$ & \\
kv128 & gate acc & $0.01\pm0.00$ & $0.04\pm0.00$ & $0.60\pm0.06$ & $\mathbf{0.95\pm0.01}$ & \textbf{4000 ($+0.004$, tie)} \\
\bottomrule
\end{tabular}}

\noindent The wall beyond kv$64$ is an \textbf{epoch-budget wall} (required epochs scale with kv: kv$32$ saturates by $\approx1000$, kv$64$ by $\approx2000$, kv$96$/kv$128$ by $\approx4000$), not a hard capacity limit. \textbf{Over-train (negative):} kv$32$ gate drops $1.00\to0.89$ from $2000\to4000$ ep. \textbf{Non-monotone fidelity dip (negative):} kv$96$ fidelity $0.39\to\mathbf{0.24}\to0.97$, kv$128$ $0.40\to\mathbf{0.09}\to0.71$ --- a transient PCD/buffer instability at intermediate epochs; do not read the $1000$-ep point as monotone progress.

\paragraph{C.4.c --- Stress axes, $2000$ ep ($n=5$).} $\Delta=$ gate $-$ Hopfield; \textbf{CLOSE} iff $\Delta\ge-0.01$.

\noindent\emph{Correlation-$\rho$ axis:}
...
\midrule
N128/kv32 & $0.5$ & $1.00$ & $1.00\pm0.00$ & $0.98$ & $+0.02$ & $10.0\times$ & \checkmark \\
N256/kv64 & $0.5$ & $1.00$ & $0.99\pm0.01$ & $0.97$ & $+0.02$ & $9.3\times$ & \checkmark \\
N384/kv96 & $0.5$ & $0.97$ & $0.90\pm0.01$ & $0.95$ & $-0.05$ & $5.5\times$ & \textbf{no} \\
N128/kv32 & $0.9$ & $0.97$ & $0.87\pm0.06$ & $\mathbf{0.72}$ & $+0.16$ & $5.6\times$ & \checkmark \\
N256/kv64 & $0.9$ & $0.89$ & $0.67\pm0.07$ & $\mathbf{0.59}$ & $+0.08$ & $2.6\times$ & \checkmark \\
N384/kv96 & $0.9$ & $0.68$ & $0.36\pm0.05$ & $0.52$ & $-0.16$ & $1.3\times$ & \textbf{no} \\
\bottomrule
\end{tabular}}

\noindent $\rho=0.5$ is identical to $\rho=0$. At $\rho=0.9$ Hopfield itself collapses ($0.72$/$0.59$/$0.52$) so the reversal \emph{widens} at kv$\le64$ --- Hopfield falling, not CLU rising (CLU gate also drops).

\noindent\emph{Eval-noise-$\sigma$ axis --- \textbf{THE NOISE WALL} (the dominant negative):}
...
\paragraph{F.6 The specified discriminating experiment (future work).} Three arms (certified mixture kernel at $\gamma=0$ with coset projection; the learned-$\tau$ escalation gate of \S4.1; naive squeeze retries) on a trained CLU associative memory, matched on FLOPs, with \textbf{the latch-erosion decay curve as the money plot} --- predicting that naive certified retries erode the register at $D=\half s^2$ while the coset-projected kernel holds it flat. Risks: (a) the erosion rate is derived on toy EBMs and a trained-$V_\theta$ coset is only approximately flat; (b) the FDT $\sigma^\star$ assumes the damped-Verlet noise model, which \texttt{langevin\_noise=legacy} runs violate; (c) matched-FLOPs accounting must charge the MALA gradient. \textbf{Partially answered, and on a different substrate:} the \emph{mechanism} half --- directed re-launch versus equal-energy random kick, ensemble-of-$k$ restarts and ungated retry-all --- is measured on a \textbf{designed} store (\S5, design rule~2; MNIST). \textbf{What remains unrun is this specification itself: the certified mixture kernel at $\gamma=0$ with coset projection, on a trained CLU checkpoint, with the latch-erosion decay curve. No trained-CLU-checkpoint result is claimed.}
