...
footnote. ⭐ **That store-versus-table statement is not confined to this rig:** it reproduces on a
real-image stream census (Split-CIFAR-10, address dimension `d = 12`, three encoder arms × 3 seeds,
pre-registered, nothing tuned), where **0 of 9 cells** show any daylight for the store over a table of its
own keys — with the store holding **2,364× the table's bytes**. ⚠ That row matches *items*, not bytes, and
it is a component-build measurement rather than a benchmark result (§4.8). **(ii)** The control the field would
...
(R² = 0.995; every `d/s` in this paper names its ruler and is computed under the subtraction convention
of §4.6, where the fitted width is flagged for a check whose known direction would only strengthen the
suppression) — **exponentially suppressed by the very admission gate that keeps the store writable**.
...
Dao & Gu, ICML 2024, PMLR 235:10041–10071) run a scalar-gated linear recurrence over that state. Titans (Behrouz et
...
byte budget and see who wins.** That comparison is not exotic. It is standard practice one field over. In
learned data structures the learned model's own bytes are charged to the learned side: a learned Bloom
filter counts as an improvement only if *"the size to represent the learned function f and the size of the
smaller backup filter for false negatives is smaller than the size of a corresponding Bloom filter with the
same false positive rate"* (Mitzenmacher, NeurIPS 2018) — i.e. the *verdict itself* is conditioned on
matched space — and learned-index benchmarks report the learned structure's own space overhead in the same
table as its lookup latency, so a learned-vs-classical comparison is **space-accounted as a matter of
course** (Kipf et al., 2019, NeurIPS ML-for-Systems workshop, non-archival; and the accompanying PVLDB
study, Marcus et al., 2020). The same discipline is recognised in evaluation methodology under the name
*partial-input baselines* (Poliak et al., 2018; Feng, Wallace & Boyd-Graber, 2019). We are importing an
established discipline into a family that has not adopted it. We are not inventing it, and §5 says so
before a reviewer does.

⚠ We keep the two halves of that precedent distinct, because they are not the same convention: only the
learned-Bloom-filter line makes *equal space* a condition of the verdict; the index benchmarks *report*
space beside speed without equalising bytes. This paper's protocol takes the stronger of the two — bytes
are equalised, not merely reported.
...
1. **A uniform matched-byte audit protocol for bounded-state memories** (§2), with five mandatory columns
   ...
   unless the selection split is fixed too. ⭐ **A third finding of the same species, from running this
   protocol's admissibility discipline on real streaming venues** (Appendix R.3): **plain prequential
   evaluation at an `h`-step-ahead horizon is not a neutral control** — it hands a continuously-updated
   learner up to `h − 1` steps of future labels, and the leak is asymmetric in the flattering direction
   (**+10.9 %** to a 250-exemplar k-NN against **−0.3 %** to a periodically-refitted GBDT). ⚠ Measured on
   one stream (Metro, 34,848 hourly pairs, `h` = 24) with one feature construction: **the mechanism is
...
7. **The same admissibility discipline applied outside this rig, and the venues it retires** (§2.5,
   Appendix R.2). Run as a pre-build tripwire against the two best-documented real streaming venues
   available to us — the INSECTS benchmark streams and the Metro Interstate Traffic Volume stream — it
   fires on both: *at laptop byte budgets an exemplar store at matched bytes sits at or above the
   strong-baseline frontier on both of the best-documented real streaming venues — and on one of them,
   destroying temporal order does not hurt it.* ⚠ *Frontier* here means **within the pre-registered
   admissibility margin**, and the two venues fire differently: on INSECTS the exemplar store lands
   **1.90 points below** ARF-100, 0.10 points inside the registered threshold (*at ARF's shoulder*, never
   beating ARF); on Metro it beats every strong reference outright, by 4–25 %. ⚠ We read this as **a
   fallback being retired, not a venue crisis**, and the runs are **baseline-only**: no memory cell of any
   kind — ours or a rival's — ran on either venue, and no benchmark result is claimed for any memory,
...
provenance in Appendix K. The figure is rendered from banked artifacts at a uniform `n = 9` and carries
**all seven bars** — the six rival arms and the CLU — with each bar's gate status read from the same
banked selection columns as §4.1.1.)
...
⭐ One consequence of that scope cuts in the favourable direction and we state it in the same place, so
that a null dividend is not over-read: **a store that organises well and then reads like a cheap table
scores dividend ≈ 0 by design — an inference-cost win, not a failure.** This paper measures the dividend
and reports whatever it is; it does not treat a null dividend as a verdict on how the content came to be
organised, which the launder never tested.
...
Three further boundaries, all hard:

- **These laws govern the store; end-to-end performance additionally depends on the encoder, measured
  separately, φ-bytes ledgered.** Every arm of every cell here — memory, launder, `+0 B` readers, null,
  blank — sees the identical read-in encoder output, asserted by content hash (§2.1), and the encoder's
  bytes are ledgered on every arm including the launder. A store-level result in this paper is therefore
  a statement about the store at a fixed encoder: a null here is **not** an end-to-end refutation, and a
  law here is **not** an end-to-end guarantee, in either direction.
...
⭐ **Both boundaries now have measured companions from outside this rig, and they tighten rather than
relax them (folded results; Appendix R).** Three measurements of ours, made on other rigs of this
research program, are folded into this revision as protocol evidence, each with its own flag-provenance
table (Appendix A.5–A.6): **(i)** the audit's central store-versus-table statement — a store whose reads
are statistically indistinguishable from, **or below**, a small table's (in this audit that comparison is
the dividend column of §4.1.1: **−0.0561 ± 0.0315**, 1.78 SE, the launder *reads no worse than* the store)
— reproduces at a **third substrate** (a third in this paper's own counting: after this audit's two
designed families, §2.5), a real-image Split-CIFAR-10 stream-census rig at address dimension `d = 12`
(3 seeds × 3 encoder arms, pre-registered, nothing tuned; §4.8, Appendix R.1); **(ii)** the venue-admissibility discipline of §2.5, run against the
two best-documented real streaming venues we could obtain, **retires both** — a byte-matched exemplar
store sits at or above the strong-baseline frontier on each (§2.5, Appendix R.2) — which is why this
paper's real-data value legs are declared NOT-RUN rather than reported (Appendix J); and **(iii)** a
benchmarking-methodology finding from the same runs — a label leak in plain prequential evaluation at
multi-step horizons, asymmetric in the flattering direction (Appendix R.3). None of the three adds a
benchmark result for any memory, ours or a rival's; no memory cell of this audit ran on real data.
...
⚠ **One statistical disclosure about the gates themselves, so that it is not extracted from us.** Every
2 SE decision in this paper — the rescue gates, the twenty (arm × head-width) frontier cells, the three
selection rules — is reported **per cell**; **no family-wise or multiplicity correction is applied, and
none is claimed**. Two things keep that from being a hidden degree of freedom: the headline quantity is
not a gate decision at all (it is each arm's own loss to its own byte-matched table, §4.2, which is
quotable for every arm whatever its gate status), and where a gate decision is fragile we print it as
fragile — INIT-UNSTABLE, SELECTION-DEPENDENT — rather than resolving it in either direction.
...
tradition (Poliak et al., \*SEM 2018; Feng, Wallace & Boyd-Graber, ACL 2019), applied here to a *memory's
...
handicapped by **0.276 / 0.263 / 0.425 / 0.856 / 0.942 / 0.614 neg-MAE** (TTT-Linear · TTT-MLP ·
DeltaNet · GDN · GDN-2 · Mamba-2, nine seeds, every one more than 2 SE from zero — the SSD arm's gap is
**+0.6139 ± 0.1386**, tabulated with the rest at Appendix I.1c(b)) relative to a **raw-metric** table
holding the same bytes, for two reasons visible at equation level: (i) `θ_K, θ_V` are trained for the *recurrence*, not for
...
⭐ **The admissibility discipline of this section has also been run outside this rig — against real
streaming venues — and it fires there too (folded protocol evidence; full detail, riders and provenance
in Appendix R.2 and A.6).** Before committing a store build to a public stream, we ran the same class of
tripwire — *is a trivial, byte-matched exemplar store already at the strong-baseline frontier?* — on the
two best-documented real streaming venues available to us: the INSECTS benchmark streams (prequential
accuracy, window 1000, 79,985 scored instances) and the Metro Interstate Traffic Volume stream
(prequential 24-hour-ahead MAE, 34,848 scored pairs). The finding, in the form we are entitled to state
it:

> **At laptop byte budgets an exemplar store at matched bytes sits at or above the strong-baseline
> frontier on both of the best-documented real streaming venues — and on one of them, destroying
> temporal order does not hurt it.** ⟦registry: N294 / N295⟧

⚠ **Decoding *frontier*, because the two venues fire differently and the sentence above is the summary of
both.** *Frontier* means **within the pre-registered admissibility margin**, not *ahead of the best
baseline*. On INSECTS the byte-matched exemplar store lands **1.90 points below** the strong reference
(ARF-100) — 0.10 points inside the registered threshold, which is why the honest sentence there is
***"the byte-matched exemplar store is at ARF's shoulder"*, never *"it beats ARF"***. On Metro there is no
threshold call to make: the exemplar store beats **every** strong reference outright, by 4–25 %
(Appendix R.2).

We read this as **a fallback being retired, not a venue crisis**: the natural fallback — *"move the
audit to real streaming data"* — does not escape the substitute-audit discipline any more than the
designed families above did, and neither venue is disqualified as a *mechanics* substrate by anything
measured here; what neither can host at these byte budgets is a claim that a memory's dynamics beat its
own stored content. ⚠ The mechanics-substrate reading is a **scope statement about what the tripwire
tests**, not a measurement: the tripwire gates the value venue, and it says nothing about either stream's
usefulness for mechanics work (Appendix R.2 states which half of it is a registered ruling of ours and
which half is our own reading). It is
also a second, independent confirmation of this section's one design rule, now measured on real venues:
both fail because the answer *is* in the table — on INSECTS a ~500-exemplar recency window is the hidden
regime variable, and on Metro the regime clock is fully encoded inside each pair's own 24-lag feature
window, so a stored exemplar never goes stale (Appendix R.2). ⛔ Nothing in those runs is a benchmark
result for any memory: they are **baseline-only** admissibility measurements, run before — and instead
of — any store build. A methodological by-product of the same runs (a label leak in plain prequential
evaluation at multi-step horizons) is reported in Appendix R.3.
...
⚠ **Ruler note for this table.** Every `d/s` in the rows above is a **designed** geometry's
architecturally-specified width — these are verification cells (§1.3), not fits — so the fitted-width
estimator, its `α‖q‖²` subtraction convention and the check that estimator is still owed (§4.6) do not
enter here, and no number in this table is comparable to a fitted `d/s` without that conversion.
...
**NOT-RUN reasons, stated once.** *Titans:* peer-reviewed at NeurIPS 2025, but no
...
⚠ **And the objection the family's own construction note invites, answered rather than left standing.**
Appendix N.1 says of the dividend family that *"the family ships with the 2-NN mean as its own strongest
control, and that control is expected to win. It does."* That is a **design property** of the family — the
answer is constructed to be absent from the table, so an aggregating reader is the obvious beneficiary —
and it is not the finding. The finding is everything the construction did **not** fix: the size and
stability of the margins, the *sign* of the same arms against the weaker projected control, the
rescue-gate outcomes (including our own arm's), and the direction in which every registered stress moved
the result. Appendix N.1 itemises what could have come out differently and did not, in one place, so that
the concession and its answer are read together.
...
⭐ Three estimates of the learned well width — by **two independent methods**, two slope fits of the same
ratio law and an independent well-fit of the store — agree to **0.7 %**. ⚠ **And this corrects one of
...
⚠ **The convention every `d/s` number in this paper is computed under, and the check that value is still
owed.** The effective-`s` estimator fitted above subtracts the store's own confining term `α‖q‖²` (§3.1)
before fitting the well width; a `d/s` figure computed without that subtraction is a different quantity
and is not comparable to these. ⛔ **On that convention `s = 0.40` is a value we flag for a check, not one
we discharge in general:** the re-measurement that would settle it is a **declared NOT-RUN** (Appendix J),
and we report it as flagged rather than as confirmed. ⭐ Its direction, however, is known rather than
guessed, and it runs against the flattering side: applying the correction makes `s` **smaller** and every
`d/s` **larger**, i.e. the coupling *more* strongly suppressed. Every suppression number printed in this
section and in §4.6.1 is therefore the **conservative** one, and the check we owe cannot move any of them
in the direction that helps our own claim.
...
⚠ **Ruler and convention, carried with the paragraph rather than left upstream.** Both `d/s` figures in
the paragraph above name their ruler (atom-width **4.3**; fitted-width **3.59**), and both are computed
under the subtraction convention stated in §4.6 (the estimator subtracts the store's own `α‖q‖²` term).
⛔ The width itself, `s = 0.40`, is **flagged for a check and not discharged in general** — the
re-measurement is a declared NOT-RUN (Appendix J) — and the correction's known direction (`s` smaller,
`d/s` larger) makes the printed suppression the conservative reading.
...
## 4.8 The same store-versus-table indistinguishability at a further, real-image substrate (folded evidence)

*(Grade: **evidence** — a component-build measurement on real-image streams, made on this program's
census rig and folded into this revision; provenance in Appendix A.5, full detail and riders in
Appendix R.1. ⛔ It is **not** one of this paper's matched-byte cells, and no number from it enters any
other section.)*

The audit's central store-versus-table statement — a store whose reads are statistically indistinguishable
from, **or below**, a small table's (in this audit that comparison is the dividend column of §4.1.1,
**−0.0561 ± 0.0315** at 1.78 SE: the launder *reads no worse than* the store) — has now been measured at a
further substrate, unrelated to both of this paper's designed families and a **third** in this paper's own
counting (after the two designed families of §2.5): a **real-image** Split-CIFAR-10 stream-census rig at
address dimension `d = 12` (a 32,768-atom store of the
same implementation class as Appendix P's, 16-item capacity, three encoder arms × 3 seeds = 9 cells,
128 cue queries per cell). The registered finding, quoted in its approved form:

> **Branch (b), 0/9 cells, 3/3 arms, pre-registered at Q6 = 0.70 with nothing tuned; |A3b| ≤ 0.047 on
> 7/9 ⇒ the store and an 832-byte table are statistically indistinguishable on held-out stream reads —
> and the null is ATTRIBUTABLE (store not inert 9/9, geometry GO and gate validation both verified in
> advance).** ⟦registry: N276⟧

Decoding the registered terms: *branch (b)* is the pre-registered **no-daylight** branch of a two-branch
registration in which both branches were declared reportable before the first cell; *Q6 = 0.70* is the
registered prior probability assigned to that branch in advance (the daylight branch carried 0.15);
*A3b* is the store-minus-table margin on **held-out stream reads** (pooled binomial SE; its cue-side
sibling A3a uses a McNemar SE); and the daylight rule, quoted from the code that computed it, is
*"(a) DAYLIGHT iff a launder margin is POSITIVE beyond 2 SE on ≥ 3 seeds — cue (A3a) or stream (A3b);
(b) NO DAYLIGHT otherwise."* Measured: **0/9 cells, 0/3 seeds on every arm, on both legs** (Appendix
R.1 carries the per-arm numbers).

Three riders travel with this row and are stated here rather than in a footnote:

- ⚠ **Matching is matched-items, not matched-bytes — and the byte asymmetry runs in the store's
  favour.** The store occupies **1,966,848 B against the 832 B cue-side table (2,364×)** — 4,728×
  against the 416 B ring table, 3.147× with the encoder priced on both sides — **and still buys no
  daylight** ⟦N276⟧. This row is therefore a *weaker-instrument sibling* of this paper's matched-byte columns, not
  one of them: the store was given a three-orders-of-magnitude byte advantage and read no better than
  the table.
- ⚠ **The ceiling itself was expected; the measured content is the attributable absence of daylight.**
  On a metric-native cue protocol a nearest-neighbour reader of the store's own keys is the Bayes-rule
  reader, so a table *ceiling* is a theorem there, stated in advance — which is exactly why the null
  branch carried the 0.70 prior. What the row adds is the null's **attributability**: the store is
  demonstrably not inert (well-depth medians 0.47–0.97 across all 9 cells), the address geometry was
  verified GO in advance (σ_q/spacing 0.334 → 0.210 at `d = 12`, 3/3 seeds), and the instrument's
  validation was verified in advance — so the null cannot be dismissed as a dead store, a bad substrate,
  or a blind instrument ⟦N276⟧.
- ⚠ **A component-build measurement, not a verdict.** It carries no tier-level claim, no performance or
  accuracy claim, no benchmark entry, and it adjudicates nothing between its encoder arms (that
  comparison is out of this paper's scope and is quoted nowhere in it).

At that substrate, under a rule registered before the first cell, with nothing tuned, the store and a
table of its own keys read the same. That is this paper's closing sentence — *a store organised well
enough to be safe is organised well enough to be a table* — measured on a rig that was not designed to
test it.
...
with a learned `M₀`.
**State-space memories.** **Mamba-2** (Dao & Gu, ICML 2024, PMLR 235:10041–10071, arXiv:2405.21060)
carries a
...
⭐ **The delta-rule line's own authors place it inside their family**: Gated DeltaNet (Yang, Kautz &
Hatamizadeh, ICLR 2025) introduces Mamba-2 as *"a simple gated update rule, `S_t = α_t S_{t−1} + v_t k_tᵀ`,
which uniformly decays all key-value associations at each time step by a dynamic ratio, `α_t ∈ (0,1)`"* —
the erase-free degenerate case of the delta rule. ⚠ **That presentation carries its authors' own hedge,
and we carry it too:** they write the recurrence in that form *"up to specific parameterization"* (their
§2.1). Mamba-2's shipped parameterisation is a per-head `P × N` structured state, not literally a
scalar-decay linear-attention recurrence; the common notation is theirs, and the identity our arm asserts
is stated and tested in Appendix O.2b rather than inherited from that sentence. With that caveat, it is
exactly what our SSD arm computes, which is why its row's cleanest reading is **what the delta-erase term
buys at byte-identical state: 0.003 ± 0.037 of `full`, i.e. nothing measurable** (§4.1.1). **Theory:** Wang, Shi & Fox (arXiv:2501.12352) unify linear attention, SSMs,
...
> routine **outside** this family: at matched or explicitly accounted space in learned data structures
> (learned Bloom filters, where the learned model's own bytes are a *condition on the verdict*; learned
> indexes and sketches, where the structure's own space is a *reported column* beside its speed), and —
> concurrently with this work — as a token-matched recency window in LLM-agent memory evaluation. We therefore position this audit as **importing an established
...
- **Audit-at-equal-bits is standard outside this family.** Learned Bloom filters (Mitzenmacher, NeurIPS
  2018) and learned-index benchmarks (Kipf et al., 2019; Marcus et al., PVLDB 2020) report the learned
  structure's own space and build cost next to its speed — the first as a *condition* on the verdict, the
  second as a *reported column* — and later SOSD-derived studies push the accounting further:
  Chesetti & Pandey (ACDA 2025) find that *"the RadixSpline and RMI are an order of magnitude (30 − 80×)
  larger than B-trees"* and that *"all learned indexes take at least 4 orders-of-magnitude more time to
  build than B-trees"*. ⚠ **Both quotes carry their scope with them, and we print it:** the subject is
  RadixSpline and RMI specifically — the same paragraph reports the PGM index as **4× *smaller*** than the
  B-tree — and that B-tree baseline is deliberately sparsified (*"we only build the B-tree by uniformly
  sampling every 256th key"*), so the ratio is measured against a shrunken classical baseline; the quotes
  are pinned to arXiv:2407.00590v2 §6.6 (the SIAM camera-ready is paywalled and we did not read it), and
  that paper's own headline is a rough **tie** in external memory, not a rout. ⛔ **The benchmark suite
  itself reaches the opposite conclusion on size** — SOSD reports size overheads of RMI 3 % and
  RadixSpline < 1 % against a B-tree's 16 %, and concludes that *"learned models indeed often outperform
  state-of-the-art implementations"*. We cite the adversarial study for the **tone of the accounting**, not
  for a verdict against learned structures; the tone is what this audit imports. What is absent in all of
  it is any sequence, any test-time dynamics, and any state.
- **The substitute audit is not ours in general form.** It is the partial-input / trivial-baseline audit
  tradition: Poliak et al. (\*SEM 2018) train on hypotheses alone and **significantly outperform** the
  majority-class baseline on **six of the ten** NLI datasets; Feng, Wallace & Boyd-Graber (ACL 2019) supply
  the caveat we carry with it. What we
...
anchor-versus-family objection. ⭐ One folded measurement sharpens how hard that bar is rather than lowering
it: the same admissibility discipline, run against the two best-documented real streaming venues,
retires both (§2.5, Appendix R.2) — the thinness is a property of the bar, now measured on designed
*and* real venues, not an idiosyncrasy of this rig's constructions.
...
gates the transfer of every geometric domain statement** — `s/sep`, `d/s`, `s_max/σ_q`. §4.6 supplies a value
*for this store only* (`s = 0.40`, three estimates by two independent methods, 0.7 % apart), computed
under a declared subtraction convention: the effective-`s` estimator subtracts the store's own `α‖q‖²`
term, and a figure computed without that subtraction is a different quantity. ⛔ **That value is flagged
for a check, not discharged — for this store or in general:** the re-measurement under the same convention
is a **declared NOT-RUN** (Appendix J). ⭐ The correction's direction is known in advance and runs against
us: it makes `s` **smaller** and every `d/s` **larger**, i.e. the third-party coupling *more* strongly
suppressed, so every suppression number in §4.6/§4.6.1 is the conservative reading. And every `d/s`
statement here names its ruler. (iii) T5.4's coupling list is **not proven exhaustive**.
...
transfers to a language-model claim**, and no language-modelling run was sized or attempted. ⚠ The
folded measurements of Appendix R do not change this: the two real-streaming-venue runs are
baseline-only admissibility measurements with **no memory arm of any kind** (theirs or ours), their
value legs are declared NOT-RUN for want of an admissible venue (Appendix J), and the third-substrate
row (§4.8) is a 3-seed component-build measurement on a different rig — not a language-model claim and
...
learned store (ruler named and subtraction convention declared at §4.6; the fitted width is flagged for a
check whose known direction makes the suppression stronger, not weaker), which means it is exponentially
suppressed by the same admission gate that keeps the store writable: at the spacing our configuration achieves it is two orders of magnitude below the query's own
launch noise.

⭐ **Three measurements from outside this rig, folded into this revision, bear on how far that conclusion
travels — and each tightens the paper's own boundaries rather than relaxing them (Appendix R).** The
store-versus-table indistinguishability reproduces on a **real-image stream census** at address dimension
`d = 12`, pre-registered with nothing tuned, in **0 of 9 cells**, with the store holding **2,364×** the
table's bytes and still buying no daylight — matched *items*, not matched bytes, and a component-build
measurement rather than a verdict (§4.8). The admissibility discipline of §2.5, run as a pre-build
tripwire on the two best-documented real streaming venues available to us, **retires both** — which is why
every real-data value leg of this work is a **declared NOT-RUN for want of an admissible venue, never a
null** (Appendix J), and which we read as **a fallback being retired, not a venue crisis** (Appendix R.2).
And the same runs produced a third protocol finding in the species of §2.6's two: **plain prequential
evaluation at an `h`-step-ahead horizon is not a neutral control**, because it hands a continuously-updated
learner up to `h − 1` steps of future labels, asymmetrically in the flattering direction — mechanism
protocol-general, magnitudes measured on one stream and not general (Appendix R.3). ⛔ None of the three
is a benchmark result for any memory: **no memory cell of any kind, ours or a rival's, ran on either
venue**, and the census row is not one of this paper's matched-byte cells.
...
| ✅ citation provenance | **discharged.** The arm's venue/year/identifier are double-sourced to the published record — **Dao & Gu, ICML 2024, PMLR 235:10041–10071, arXiv:2405.21060** (PMLR proceedings page + arXiv). ⚠ Author order: **Mamba-2 = Dao & Gu**, Mamba-1 = Gu & Dao. The reference-implementation state convention (**per head `d_head × d_state`**, i.e. the `P × N` special case our arm instantiates at `d_state = head_dim = 36`) is triple-sourced to the official repository's `mamba_ssm/modules/mamba2.py` and `mamba_ssm/modules/ssd_minimal.py` plus the authors' companion post. ⚠ The Mamba-2 **paper body text** was not read in the verification pass (PDF/HTML tooling failure); the state convention rests on the reference implementation, which is the primary artifact for an implementation convention, and on the authors' own description — it is *not* verified against the paper's prose. The reference default `chunk_size = 256` (which this rig deviates from at 16) is confirmed from `mamba2.py` |
...
### A.5 The third-substrate no-daylight row (§4.8, Appendix R.1) — folded, not re-run

| item | value |
|---|---|
| what it is | ⭐ a **folded** result: measured on this program's census rig and quoted here in its registered form; **nothing was re-run for this paper** ⟦registry: N276⟧ |
| measuring commit / branch | `0f057a0` on `c2w8p3-capture-strong-phi` (base local `main @ 18b4205`, dedicated worktree); the shipped HEAD `d7eabdd` touches tests only and changes no number |
| environment | main venv reused (no worktree re-sync), **JAX 0.9.0**, float32; every run executed with cwd = the worktree and a printed module-provenance line per cell |
| substrate | **Split-CIFAR-10, reduced protocol** (5 tasks × 2 classes, class-incremental, 1000 train / 500 test per task); store `addr_dim = 12`, `payload_dim = 1`, **32,768 atoms**, `capacity = 16`, `well_budget = 8`, `leak = 0.02`, `write_steps = 300`, `read_steps = 800`, `address_steps = 400`, 128 cue queries/cell |
| store width flag | `atom_width_frac_spacing = 1.5`, co-scaled to each seed's own measured spacing (⚠ the **banked census value**, quoted with its instrument; the shipped config default differs, and this cell is **not** asserted reproducible at shipped defaults ⟦N290⟧); `atom_kernel = wendland` (cutoff 2.5), `atom_site_local_init = True` |
| encoder arms | a fitted contrastive encoder (`phi_dim = 256`, 8,000 fit steps — "strong" only by the CL-accuracy metric that defines it on that rig) · an **unfitted** random-convolution control (`phi_dim = 256`, 0 fit steps) · a PCA reference at `phi_dim = 12`. Stream fingerprints are bit-identical across arms per seed ⇒ every cross-arm quantity is paired |
| seeds | **0, 1, 2** per arm = 9 cells |
| registration | the two-branch daylight rule and both branches' reportability registered **before the first cell**; branch priors **Q6 = 0.70** (no daylight) / Q5 = 0.15 (daylight); **nothing tuned** — store arm, map, dimension and seeds all fixed in advance |
| instrument label | measured under the **pass-3 completed-gate instrument, before that rig's close-out hardening**; per the program's labelling rule, numbers stay labelled with the instrument that produced them. The daylight rule quoted in §4.8/R.1 is the registered A3 rule, quoted from the code that computed it |
| bytes | matched-**items**, ⛔ **not** matched-bytes: store **1,966,848 B** vs **832 B** (cue table, `n = 16`) = **2,364×**; 416 B (ring table, `n = 8`) = 4,728×; **3.147×** with the encoder priced on both sides (fitted arms) / 13.278× (PCA). ⛔ No leg of that instrument reads bytes |
| launder audit | **asserted, not intended**: the table re-derives its keys from the projected encoder output and raises unless bit-identical — 9/9 cells `launder_key_dim = store_address_dim = 12`, bit-identical to the store's addresses |
| wall | 27,043 s of census cells (7.5 h) + 2,373 s scale control, on the census rig at its own date |

### A.6 The two streaming-venue gate runs and the label embargo (§2.5, Appendix R.2–R.3) — folded, not re-run

| item | value |
|---|---|
| what they are | ⭐ two **folded, baseline-only** venue-admissibility runs (⛔ **no memory cell of any kind — no CLU, no rival**) plus the protocol finding they produced ⟦registry: N294 / N295 / N296⟧ |
| repo state | HEAD `7fcef50`, unchanged before and after; 0 tracked files modified |
| environments | dedicated scratch venvs — CPython 3.12.9, `river 0.25.0`, numpy 2.5.2, scikit-learn 1.9.0, scipy 1.18.0 (INSECTS); the same plus torch 2.13.0, matplotlib 3.11.1 (Metro); the project venv and lockfile untouched; `m = 1` (undecimated) for every published number |
| INSECTS protocol | prequential accuracy, window 1000 (the benchmark authors' convention), index 0 excluded ⇒ n_scored = **79,985**; primary condition `incremental-reoccurring`, second condition `incremental-abrupt-reoccurring` (balanced) |
| INSECTS arms | exemplar stores — SAM-kNN (⚠ **our own port; the streaming library ships none**, so every "one-line baseline" cost estimate is void; validated against the authors' published Weather row, 21.70 / 21.68 % error vs published 21.74 / 21.53, and 400/400 brute-force agreement on 500 randomised kNN cases) and plain kNN windows at `L ∈ {250 … 14,782}` — each run **raw AND causally standardised with the max consumed** (a declared, pre-registered anti-hobbling rule); strong reference — **`river` 0.25.0's `ARFClassifier` at 100 trees**, 3 seeds (**78.8139 ± 0.0526 SD** vs the published **77.13**, which is a **MOA** number at an **unstated** ensemble size — 100 is MOA's default and reading their arm as ARF-100 is our inference; ⚠ **`river` ARF ≠ MOA ARF**, and ours is the stronger of the two, the direction that makes firing *harder*). ⚠ ARF's measured state is **9,542,925 B = 14.35×** SAM-kNN's 665,000 B ⇒ **ARF is byte-matched to nothing** and is the accuracy frontier only |
| Metro protocol | hidden clock (`date_time` withheld from every arm); 24-h horizon; **24-h label embargo ON for every primary number** (Appendix R.3); prequential MAE/RMSE over all **34,848** scored pairs; **32 features = 132 B/exemplar** (24 recent lags ⊕ 3 weekly echoes ⊕ 4 weather ⊕ holiday), identical for every arm (⚠ the feature vector is the analyst's declared choice, and the k-NN attack is defined by it; mitigations: the strong baselines get exactly the same features, and a static 70/30 chronological holdout reproduces the parity outside the harness); gap-fill ≤ 3 h on features only (6.4 % of feature hours; **targets never imputed**; ⚠ the dataset's own repository record reports *no missing values*, which is consistent: it means no missing **cells** in the 48,204 delivered rows, whereas our gaps are missing **hourly rows** on the reconstructed hourly grid); ⚠ a 7,386-hour (10-month) sensor hole makes the stream two eras |
| Metro arms | exemplar k-NN over past windows at `L ∈ {250 … 34,847}` (**registered `k = 5`**; a declared anti-hobbling k-ladder beside it, labelled at every quotation); **nine** strong references (`gbdt_tuned` · `gbdt` · `gbdt_cat` · `gbdt_recent` · `rls` · `gru_big` · `ridge_batch` · `mlp` · `gru`), single seed (0); `rls_ff` diverged ⇒ declared NOT-RUN (Appendix J) |
| seeds / determinism | exemplar arms are single deterministic runs (**no variance estimate is claimed and none exists**); ARF 3-seeded; Metro strong baselines single-seeded — declared |
| shuffle null (Metro) | fixed-seed (`default_rng(1)`) uniform permutation of the **pair sequence** — `P(X, y)` and `P(y|X)` preserved exactly, only the order destroyed; positive control exact (persistence 574.1346849611 ordered vs shuffled, diff 0). ⚠ A shuffle, **not** a drift-free data source: it preserves the pooled six-year mixture, not a stationary process |
| declared harness fixes | a causal z-score blow-up at `t < 500` fixed by a declared ±10 SD clip on every standardised arm (the pre-fix numbers reported and used nowhere) ⟦N296⟧ |
| artifacts | `BENCHMARK-GATE.json` (INSECTS) · `METRO-GATE.json` including the 45-arm embargo delta table and `null_meta.json` (Metro) |
...
⚠ **One declaration this table does *not* contain, named here rather than left to be found.** §4.2's
optional-stopping defence rests on seeds 3–8 having been added as a **declared power addition**, and that
declaration lives in the second pass's own **run record** — its flag-provenance seeds row reads
*"0, 1, 2 (the registered primary) + 3–8 (a declared power addition)"* (Appendix A.1) — **not** in any of
the five dated documents
above: document 4, filed 2026-08-01, registers seeds 0–2. ⛔ **We therefore claim no dated
pre-registration for the seed addition**, and a reader should discount the defence accordingly. What
stands in its place is printed in full and is checkable: the addition was declared before the pooled
aggregate was computed and was not conditioned on the result; the registered three-seed primary already
gives the same headline sign on every arm (I.1a); and the verdict change that matters for our comparisons
ran **against** the flattering direction — DeltaNet became rescued, which *adds* a functioning rival arm
to compare against — while the one change that ran our way is a count on a column the headline does not
use, printed both ways rather than picked (§4.2, I.1a).
...
- **The `s` re-measurement under the `α‖q‖²` subtraction convention** (§4.6, §4.6.1, §6 L9-ii). The
  fitted well width `s = 0.40` is quoted under the declared convention that the effective-`s` estimator
  subtracts the store's own confining term; the re-measurement that would settle that value **in general**
  was not run. ⛔ It is **flagged for a check, never reported as a confirmation and never as a null**.
  ⭐ Its direction is known rather than guessed: the correction makes `s` smaller and every `d/s` larger,
  i.e. it *strengthens* the suppression claim of §4.6.1 — so every `d/s` and every coupling figure printed
  in this paper is the conservative one, and this NOT-RUN cannot be hiding a number that would help us.
...
- **The eviction-path deletion as a robustness arm** (eviction *re-draws* the freed group, so its Δ would
  not be the item's).
- **Any real-data VALUE leg for any memory — ours or a rival's.** ⛔ **Declared NOT-RUN for want of an
  admissible venue, never a null**: the two best-documented real streaming venues we gated both fail the
  venue-admissibility tripwire (a byte-matched exemplar store sits at or above their strong-baseline
  frontier; §2.5, Appendix R.2 ⟦registry: N294 / N295⟧), so no store build and no memory cell — ours or
  any rival's — was run on either. Both venues remain admissible as *mechanics* substrates.
- **A third real tabular streaming venue.** Deliberately not shopped after the two firings above — a
  registered stop, not a budget accident ⟦registry: N295⟧.
- **The INSECTS `out-of-control` stream (the venue's own drift-free null).** ⛔ It has **no data
  source** — absent from the 2024-04-16 archive and from the streaming library used — so that leg is a
  **DECLARED NOT-RUN, never a null** ⟦registry: N300⟧. The drift-free-null role is filled on Metro by
  the order-destroying shuffle (Appendix R.2), which is a shuffle of the pair sequence, not a stationary
  data source.
- **`rls_ff` (recursive least squares, forgetting factor 0.999) on Metro.** Diverged (RMSE 5.3e4, then
  2.3e5) ⇒ excluded from the reference set and declared NOT-RUN rather than reported ⟦registry: N296⟧.
- **Any language-modelling leg.** Not sized, not attempted (§6 L11).
...
> PDF twin); every plotted value traces to a named field in a machine-readable
> figure → artifact → field → value provenance table. Two declared non-artifact quantities exist and are
> logged inside the 48-entry table covering Figures 2–5: Figure 4's attention normalisation (computed with
> the artifact's own published rule) and Figure 3's shading boundary (presentational).
> ⭐ **Figure 1 was re-rendered for this revision and now carries all seven bars** — the six rival arms and
> the CLU — at a uniform `n = 9`, with its own **14-entry** provenance record; ⛔ **no cell was measured or
> re-measured for it**: every bar and every gate status is read from the same banked artifacts that produce
> §4.1.1's table, and the renderer asserts the seven-bar count and the uniform seed count before it draws.
> Figures 2–5 are the previous revision's renders, unchanged.
...
revision's rule (n = 9 on the rivals, n = 3 on the CLU) is superseded and must not be carried.
✅ **Rendered as specified: seven bars** (six rival arms + the CLU), uniform `n = 9`, zero line drawn,
CLU in a distinct fill and hatched NOT RESCUED.

**Figure 1 as rendered, bar by bar** (values and gate statuses read from the banked artifacts, identical to
§4.1.1's table; the gate status of each bar is *derived* from the banked selection columns rather than
typed in):

| bar | raw-metric `+0 B` margin | `n` | gate status (hatching) |
|---|---|---|---|
| TTT-Linear | −0.4602 ± 0.1038 | 9 | INIT-UNSTABLE (clears under one initialisation scheme, not the other) |
| TTT-MLP | −0.4425 ± 0.0869 | 9 | NOT RESCUED (no configuration, no selection rule) |
| DeltaNet | −0.2732 ± 0.0395 | 9 | SELECTION-DEPENDENT (fails the gate under held-out selection) |
| Gated DeltaNet | −0.2600 ± 0.0278 | 9 | rescued under all three selection rules — unhatched |
| Gated DeltaNet-2 | −0.2592 ± 0.0292 | 9 | SELECTION-DEPENDENT |
| Mamba-2 (SSD) | −0.2563 ± 0.0416 | 9 | rescued under all three selection rules — unhatched |
| **CLU (ours)** | **−0.2897 ± 0.0328** | 9 | **NOT RESCUED** (distinct fill; the caption says so in the same breath as the rival hatching) |

⛔ **Zero new measurement entered this render.** The five incumbent bars and their four selection columns
come from the nine-seed re-aggregation artifact behind §4.1.1 and Appendix I.1c; the Mamba-2 bar and its
three selection columns come from that arm's own banked nine-seed aggregation (Appendix A.1f); the CLU bar
comes from the banked CLU re-aggregation (Appendix A.1e). The renderer fails loudly rather than drawing if
the bar count is not seven or any bar is not at `n = 9`.
...
19. **The byte-frontier column resolves nothing at nine seeds either**: 0 of 15 (arm × head-width) cells
    clear the rescue gate, best lift +0.0694 ± 0.0491 (Appendix H.1 — the three-arm sub-table; **the
    column of record is 0 of 20** once the SSD arm is added, entry 23 below). The extra power did not rescue the
...
25. ⛔ **The INSECTS venue fails the admissibility tripwire, and the anti-hobbling rule is what decided
    it** (Appendix R.2 ⟦registry: N294⟧): the strong-baseline frontier clears a byte-matched exemplar
    store by 1.90 points where the pre-registered rule required more — the margin lands 0.10 points
    inside the threshold — and causal standardisation (worth +4.0 to +6.3 points to the exemplar arms)
    is what put the store at the frontier's shoulder. Had the exemplar arms been run raw only, the
    tripwire would not have fired: an audit that hobbles its trivial baseline passes its own venue.
26. ⛔ **The Metro venue fails three admissibility checks at once** (Appendix R.2 ⟦registry: N295⟧): the
    byte-matched exemplar store beats every one of **nine** strong references by 4–25 % relative MAE;
    the strong frontier's headroom over a one-line weekly rule (`ŷ_j = y_{j−168}`) is 2.17 %; and
    destroying the stream's temporal order *helps* the exemplar store at and above the audited byte
    budgets (−1.9 % to −2.9 % MAE) — at those budgets the venue's temporal ordering carries no
    exploitable information, so there is nothing there for a memory to be good at.
27. ⛔ **Our own pre-registration asserted that plain prequential evaluation is leak-free at a 24-hour
    horizon, and that assertion was refuted by mechanism** (Appendix R.3 ⟦registry: N296⟧):
    test-then-train hands a continuously-updated learner up to 23 hours of future labels, and the leak
    is asymmetric in the flattering direction — worth up to +10.85 % MAE to a 250-exemplar k-NN and
    −0.34 % to a periodically-refit GBDT. The embargoed protocol is used for every primary Metro number
    and the venue verdict is unchanged. A second defect sat in the same registration:
    seasonal-naive(`t − 24 h`) is **degenerate** at a 24-hour horizon (bit-identical to persistence,
    max |diff| = 0.0); the non-degenerate naive is `t − 168 h`.
28. **A pre-registered no-daylight null at a third substrate, kept as the null it is** (§4.8, Appendix
    R.1 ⟦registry: N276⟧): 0/9 cells show daylight for the store over an 832-byte table of its own keys
    on held-out stream reads, at a byte asymmetry of 2,364× in the store's *favour*. The null is
    attributable (store not inert 9/9; geometry and instrument validation verified in advance) — it is
    this paper's thesis measured on a rig that was not designed to test it.
...
reproducibility incident is disclosed in §6 L12. ⭐ The folded measurements of Appendix R carry their
own provenance tables (Appendix A.5–A.6) and their own seed disciplines: the third-substrate census
cells are 3-seeded per arm with bit-identical streams across arms (every cross-arm quantity paired); the
INSECTS ARF reference is 3-seeded; every exemplar arm on both streaming venues is a single deterministic
run, and no variance estimate is claimed where none exists.
...
⚠ **What that concession does and does not give away — itemised here, because the sentence above is the
sharpest one a reader can quote against this paper** (pointed to from §4.2). That the 2-NN mean is this
family's strongest `+0 B` reader is a **design property**: the family is built so that the answer is
provably not in the table, which is what stops an arg-min lookup from being accidentally right and makes
an aggregating reader the obvious beneficiary. It is a statement about the *reader ranking*, and the
audit's headline is not that ranking. Everything the construction does **not** fix could have come out
otherwise, and each of these was free to move:

- **The size and stability of the margins.** Nothing in the construction sets how far a *fitted memory*
  falls short of that reader. Measured: **−0.2563 … −0.4602** at nine seeds, every arm at least
  **4.4 SE** below zero — the sign holding on **all six** rival arms under the full grid at nine seeds,
  and on the five incumbent arms additionally at 5× the outer budget and under a held-out selection rule
  (both three-seed re-selections, labelled as such; §4.2, Appendix I.1). A family that merely *favoured* the
  table would be compatible with margins inside noise; these are not.
- **The sign of the same arms against the weaker control.** Against the projected control four of six
  arms are comfortably **positive** (**+0.1515 … +0.6824**, §4.3) — this family is perfectly capable of
  paying test-time dynamics a dividend, and it does, against the control the field would naturally build.
  Which control is run decides the *sign*, and that finding could not have been manufactured by the
  construction, because both controls were registered before any measurement (§2.4).
- **The rescue-gate outcomes, including our own.** Nothing in the construction forces an arm to sit at its
  own blank store. All six rival arms were free to clear the gate under every rule; two did (§4.1.1). Our
  own store was free to clear it and **did not** — a design that foreordained the headline would not also
  have disqualified the authors' arm at the same nine seeds.
- **The rivals' behaviour against their own projected tables.** All six arms were free to fail even that
  weaker bar; **four of six** cleared it (§4.3), which is what makes the raw-table result a statement about
  the *control* rather than about broken arms.
- **The direction the registered stresses moved things.** The widened grid, the 5× budget re-check, the
  held-out selection rule and the six added seeds were all registered as things that could change the
  paper (§2.6, Appendix I.1b). None moved the headline; the verdict change that matters for our
  comparisons ran **against** the flattering direction (DeltaNet became rescued, which *adds* a
  functioning rival arm to compare against), and the one change that ran our way is a count on a column
  the headline does not use — printed both ways rather than picked (§4.2, Appendix I.1a).
- **The pre-registration scorecard itself.** Six of our registered predictions were confirmed, four
  partial, **three wrong-direction** and two NOT-RUN (Appendix I.1), and one prediction of ours about this
  very family's rescue stability was **refuted** (§6 L2a). The misses are printed rather than re-scored.

⇒ The family was built so that a reader-discrimination question is **askable**; it was not built so that
the audit's answer is fixed in advance. The claim made from it is the one in §4.2 and no larger, and
§6 L1 states — in the paper's own voice — the thinness of resting an audit on one surviving family.
...
## Appendix Q — Verified bibliography entries

⚠ **Scope of this appendix, stated first.** This is **not** the paper's reference list. It carries the
entries whose venue, pagination, identifier and — where the draft quotes them — **quoted wording** have
been checked against a published record or a primary artifact in a dedicated verification pass, together
with the caveats that must travel with each. The full reference list is assembled at the typesetting pass;
every other citation in this draft is qualitative and quotes no number (the list of those is in the
editorial items, 6b). Entries are given as BibTeX so the caveats survive the copy.

### Q.1 The thirteen verified sources (fourteen BibTeX records)

⚠ Thirteen *sources*, fourteen *records*: `river` is cited both as the artifact actually run
(a `@misc` software entry pinned to the version) and as its paper of record, and printing only one of the
two would misdescribe what was used.

⭐ **Six of the thirteen were added at this revision**, when the citations introduced by the previous
revision's evidence fold (Souza et al. · Losing et al. · Gomes et al. · `river` · UCI Metro · Webb et al.)
were verified against primary sources in a dedicated pass: publisher/arXiv/official-docs primary,
aggregators as corroboration only and never alone. Three usage qualifiers that pass produced are applied
in the body of this draft and named here so the trail is short: the **Webb** attribution is split
(framework theirs, the total-variation instantiation ours — §R.2.2), the **ARF** anchor is labelled
**MOA-vs-`river`** with the ensemble size marked as our inference (§R.2.1, A.6), and the SAM-kNN
**3,000-exemplar STM cap** is relabelled as a reference-implementation default rather than a published
one (§R.2.1).
...
@misc{kipf2019sosd,
  ...
  note         = {NeurIPS 2019 Workshop on Machine Learning for Systems (NON-ARCHIVAL; DBLP
                  has only the CoRR entry -- cite the arXiv id). Verified 2026-08-01 (arXiv
                  abs Comments field + full PDF read). CAUTION: this paper does NOT contain
                  the "30-80x larger" or "4 orders-of-magnitude" claims; its Table 2 size
                  overheads are RMI 3%, RadixSpline <1% vs B-tree 16%, and its abstract says
                  learned models "often outperform state-of-the-art implementations".}}
...
@article{marcus2020benchmarking,
  ...
  note    = {arXiv:2006.12804. Venue/volume/pages/DOI verified 2026-08-01 (DBLP + vldb.org
             asset path). FIRST AUTHOR IS MARCUS, NOT KIPF, and the author list gains Misra
             vs. SOSD -- this entry and kipf2019sosd must never be collapsed. Body text NOT
             read from the published PDF -- do not quote its numbers without a re-check.}}
...
@inproceedings{chesetti2025learnedjoins,
  ...
  note      = {arXiv:2407.00590 (v1 30 Jun 2024, v2 7 Jul 2024, v3 23 May 2025). THIS is the
               ...
               read directly 2026-08-01. NOT SOSD. Caveats that must travel with the quotes:
               same paragraph reports PGM 4x SMALLER than the B-tree, and the B-tree baseline
               is built on every 256th key. SIAM camera-ready not read (paywalled) -- this
               draft quotes the arXiv v2 version explicitly.}}
...
@inproceedings{poliak-etal-2018-hypothesis,
    ...
    note      = "*SEM 2018. arXiv:1805.01042. Verified 2026-08-01 (ACL Anthology .bib +
                 ...
                 and the ACL Anthology record governs."}
...
@inproceedings{dao2024mamba2,
  ...
  note      = {Mamba-2. arXiv:2405.21060 (v1, 31 May 2024). Reference implementation:
               ...
               The paper's own body text was NOT read in the verification pass; the
               per-head d_head x d_state state convention this draft relies on is sourced
               to mamba2.py, ssd_minimal.py and the authors' companion post (App. A.1f).}}
...
@inproceedings{yang2025gateddeltanet,
  ...
  year      = {2025},
  note      = {arXiv:2412.06464. Camera-ready verified 2026-08-01 (header "Published as a
               conference paper at ICLR 2025") + ar5iv rendering + IA Scholar phrase hit.
               Quotable (Sec 1): "Mamba2 addresses this limitation by introducing a simple
               gated update rule, S_t = alpha_t S_{t-1} + v_t k_t^T, which uniformly decays
               all key-value associations at each time step by a dynamic ratio,
               alpha_t in (0,1)." Sec 2.1 hedges this as Mamba2 "up to specific
               parameterization" -- that hedge MUST travel with the quote (Sec 5.1).
               Code: github.com/NVlabs/GatedDeltaNet.}}
...
### Q.2 Entries deliberately **not** printed

- **Belinkov et al. (2019), "On Adversarial Removal of Hypothesis-only Bias in Natural Language
  Inference" (\*SEM 2019, arXiv:1907.04389).** It corroborates Poliak et al.'s setup (ten NLI datasets;
  SNLI 34 % → 69 %) but **not** the "six of ten" count, its own venue and pages were not verified, and
  this draft does not cite it. ⛔ It is therefore omitted rather than carried as an unverified entry — if
  a later revision wants the corroboration, the entry must be checked first.
- **Poliak et al.'s Table 2 decimals** and the identity of the six winning datasets: reconstructed from a
  LaTeX rendering in the verification pass, single-sourced. ⛔ Not printed anywhere in this draft, and not
  to be printed without a re-check. The **count** is what the draft quotes, and the count is
  double-sourced including the published proceedings text.
- **The 2019 correction to Gomes et al.** ("Correction to: Adaptive random forests for evolving data
  stream classification", *Machine Learning* 108(10):1877–1878, 2019, DOI `10.1007/s10994-019-05793-3`).
  Its record is verified; **its contents were not read**. It is inert here because this draft quotes **no
  number** from Gomes et al. — every ARF number in §R.2.1 and A.6 is either ours or the INSECTS benchmark
  authors' measurement of ARF. ⛔ It must be read before any ARF number is ever quoted from that paper.
- **The provenance of `ltm_size = 0.4`** (the SAM-kNN reference-implementation default behind the
  3,000-exemplar STM cap, §R.2.1). Confirmed as a documented default of the reference implementation, but
  **single-sourced on whose default it is**; the load-bearing half — that the parameter is **absent from
  the published paper**, which publishes `k`, `L_min` and `L_max` only — is verified from the paper
  itself. Printed as a caveat inside the `losing2016samknn` note rather than as an entry of its own.

---

## Appendix R — Protocol evidence folded from outside this audit's rig

> **What this appendix is.** Three measurements made by this research program on other rigs, folded into
> this revision because each is evidence about this paper's protocol or thesis rather than a new cell of
> the audit. Each carries its internal registry marker ⟦N…⟧ (provenance, stripped at circulation;
> Appendix A.5–A.6 carry the flag-provenance tables). ⛔ **Standing scope for the whole appendix:**
> nothing here is a benchmark result for any memory — the two streaming-venue runs are **baseline-only**
> (no CLU cell and no rival-memory cell of any kind was run on either venue), and the third-substrate
> row is a component-build measurement, not a verdict. No number from this appendix enters any audit
> column, and no external benchmark is claimed as won on its own headline metric — here or anywhere in
> this paper.

### R.1 The no-daylight row at a third substrate ⟦registry: N276⟧

**The row, in its approved form** (§4.8 carries the same quotation and the decoding of its registered
terms):

> Branch (b), 0/9 cells, 3/3 arms, pre-registered at Q6 = 0.70 with nothing tuned; |A3b| ≤ 0.047 on 7/9
> ⇒ the store and an 832-byte table are statistically indistinguishable on held-out stream reads — and
> the null is ATTRIBUTABLE (store not inert 9/9, geometry GO and gate validation both verified in
> advance).

**The per-arm numbers.** Cue-side margin A3a (store − table, McNemar SE), mean ± SE per arm: fitted
contrastive encoder **−0.2526 ± 0.0456** · unfitted random-convolution control **−0.1432 ± 0.0409** ·
PCA reference **−0.1224 ± 0.0365** — on the cue side the table is *ahead* on every arm, which is the
metric-native ceiling doing what the theorem says it does (§4.8, rider 2). Stream-side margin A3b
(pooled binomial SE), mean ± SE per arm: **−0.1042 ± 0.0497 · +0.0156 ± 0.0239 · −0.0260 ± 0.0104** —
statistically indistinguishable from zero: |A3b| ≤ 0.047 on 7 of 9 cells and 8 of 9 sit inside
2 SE ≈ 0.175 (the exception is the fitted arm's seed 2 at −0.2031 ± 0.1729, on the *table's* side).
Under the registered rule — daylight iff a store-over-table margin is positive beyond 2 SE on ≥ 3 seeds,
on either leg — the count is **0 of 9 cells, 0/3 seeds on every arm, both legs**. Visible in the raw
stream reads: the control arm's seed 0 gives store **[0.6875, 0.5000, 0.2500, 0.1875]** against table
**[0.6875, 0.5000, 0.3125, 0.2500]** — identical on the first two read events.

**The attribution, verified in advance rather than reconstructed.** The null is not a dead-store
artifact: well-depth medians are **0.94 / 0.89 / 0.73** (fitted arm), **0.97 / 0.77 / 0.86** (control),
**0.75 / 0.47 / 0.60** (PCA) — 9/9 cells not inert. The address geometry cleared its registered GO rule
before the run (σ_q/spacing **0.334 at the PCA reference and 0.210 at the fitted contrastive encoder**,
`d = 12`, 3/3 seeds — the ratio of the queries' own jitter to the spacing between stored keys, so smaller
is safer and both arms sit far below 1), and the instrument's validation was verified in advance. The table itself is audited **in code**: it re-derives its keys from the projected
encoder output and raises unless bit-identical to the store's addresses (9/9 cells,
`launder_key_dim = store_address_dim = 12`).

**Riders (each also stated at §4.8):** matched-items, not matched-bytes, with the byte asymmetry
2,364× in the store's favour (Appendix A.5); the metric-native ceiling was expected in advance (hence
the 0.70 prior on the null branch) and the measured content is the attributable **absence of daylight**;
a component-build measurement — no tier-level, performance, accuracy or benchmark claim, and no
adjudication between the encoder arms (the cross-arm comparison exists in this program's records, is
confounded in ways that require its own qualifiers, and is quoted nowhere in this paper). ⚠ Instrument
label: measured under the census rig's pass-3 instrument, before that rig's later hardening pass; the
numbers stay labelled with the instrument that produced them (Appendix A.5).

### R.2 The two real-streaming-venue admissibility firings ⟦registry: N294 / N295⟧

**The rule being applied.** The same discipline as §2.5, transported to an external venue: before any
store build, ask whether a **trivial, byte-matched exemplar store** — a kNN window over raw stored
pairs, the streaming analogue of this paper's table launder — already sits at the venue's
strong-baseline frontier. If it does, the venue cannot separate a memory's dynamics from its stored
content at that budget, and no value claim for *any* memory is hosted there. The finding, in the
approved form: **at laptop byte budgets an exemplar store at matched bytes sits at or above the
strong-baseline frontier on both of the best-documented real streaming venues — and on one of them,
destroying temporal order does not hurt it.** The ruled framing travels with it verbatim: this is
**"a FALLBACK BEING RETIRED, not a venue crisis."**

**R.2.1 INSECTS (`incremental-reoccurring` primary; `incremental-abrupt-reoccurring` second condition)
⟦N294⟧.** The harness reproduces the venue's published anchors before it measures anything: No-Change
**40.4526** vs the published **40.46** (Δ −0.0074 — a pure function of the label sequence, so any
re-ordering would move it; it does not), and ARF-100 **78.8139 ± 0.0526 SD** (3 seeds) vs the published
**77.13**. ⚠ **Those two ARF numbers are two different implementations and we name them rather than let
the comparison imply otherwise: `river` ARF ≠ MOA ARF.** Ours is `river` 0.25.0's `ARFClassifier`; the
published 77.13 is a **MOA** number, and the benchmark's authors never state ARF's ensemble size anywhere
— **100 is MOA's default, so reading their arm as "ARF-100" is our inference**, not a published
parameter. The reproduction is therefore an anchor check across implementations, not a like-for-like
replication, and nothing in this section rests on the two being the same object. The best registered exemplar arm — SAM-kNN at its own published 0.634 MiB budget
(**76.9157 %** at 665,000 B, causally standardised) — lands **1.90 points** below ARF-100 where the
pre-registered rule required more: the margin is **0.10 points inside the threshold**, so the venue
fails. It fails against **every** ARF reference computed (−1.898 3-seed mean · −1.937 best seed ·
−1.838 worst seed · −0.497 our ARF-10 · −0.214 the published 77.13), and the second condition fails at
−1.845. An unregistered *stronger* exemplar arm (SAM-kNN, L_max = 1000, **77.0632 %** at 133,000 B) sits
within **0.07 points** of the venue's **best published method** (ARF at 77.13 — ⚠ best of the six methods
the benchmark's own authors ran, which is what "best published" means here; it is not a literature-wide
state-of-the-art claim and we do not make one). ⚠ The honest sentence, and it is the one our own gate record
insisted on, is ***"the byte-matched exemplar store is at ARF's shoulder"*, never "it beats ARF."* ⚠ And ARF's
byte caveat is mandatory wherever ARF anchors the frontier: ARF's measured state is **9,542,925 B =
14.35×** SAM-kNN's 665,000 B — **ARF is byte-matched to nothing** here; it is the accuracy frontier
only.
  **Mechanism, measured:** exemplar accuracy is *monotone decreasing* in store size above L ≈ 500
(**75.56 / 76.03 / 75.36 / 73.39 / 68.15 / 59.75 %** at L = 250 / 500 / 1,000 / 2,000 / 5,000 / 14,782),
so the store byte-matched to our own planned build (14,782 exemplars = 1,966,006 B) is the **worst**
exemplar arm — 19 points below a store 30× smaller. Recency is the hidden regime variable (a
500-example window spans ≈ 0.13 °C of the venue's 20 °C temperature sweep); SAM-kNN discovers this
itself (its short-term memory averages 945 of a **3,000-exemplar STM cap**; its long-term memory is
selected on 17.7 % of instances) — ⚠ **that cap is not a published number**: it follows from the authors'
**reference-implementation default `ltm_size = 0.4`** at `L_max = 5000`, whereas the ICDM paper publishes
`k`, `L_min` and `L_max` only, and the strong reference is a recency mechanism here too (78 of ARF's
100 trees hold exactly one node at the end of the pass). On INSECTS the winning strategy is *forget
fast* — the exact inverse of a persistence venue.
  **The rule that decided it:** the frozen CSV is unnormalised, so every exemplar arm ran raw **and**
causally standardised, with the tripwire consuming the max (a declared anti-hobbling rule).
Standardisation is worth +4.0 to +6.3 points (SAM-kNN 5000: 71.52 raw → 76.92 std). ⛔ Had the exemplar
arms been run raw only, the venue would have **passed** — an audit that hobbles its trivial baseline
admits its own venue. This is §2.4's lesson (the control's construction decides the verdict), reproduced
on a real venue.
  **Caveats carried:** our SAM-kNN is our own port (the streaming library ships none — every cost
estimate premised on a "one-line baseline" is void), validated against the authors' published Weather
row and by exact brute-force agreement (Appendix A.6); the exemplar arms are single deterministic runs;
our ARF is stronger than the published one (+1.68), the direction that makes firing harder; the
standardiser is the analyst's declared, pre-registered choice. ⛔ The venue's terminal persistence band
is at ceiling and persistence-trivial, and no per-band retention or acquisition number is quoted from it
here or anywhere. ⛔ The venue's own drift-free-null stream has no data source and is a declared NOT-RUN
(Appendix J).

**R.2.2 Metro Interstate Traffic Volume (hourly, 24-h horizon, hidden clock, label embargo on) ⟦N295⟧.**
Unlike INSECTS, this firing is not a threshold call — the venue fails **three** independent checks:

1. **The exemplar store beats every strong reference outright.** The best exemplar at SAM-kNN's
   published budget (k-NN over past windows, L = 5,037 = 664,884 B; the declared anti-hobbling k-ladder
   arm at k = 10, raw) reads **MAE 314.575** against the best strong reference `gbdt_tuned` at
   **335.203** — a −6.15 % relative margin — and the **registered k = 5** arm at the same budget
   (**320.982**) fires at −4.24 % on its own. It fires against all **nine** strong references, by
   4–25 %: `gbdt_tuned` −0.0615 · `gbdt` −0.0710 · `gbdt_cat` −0.0721 · `gbdt_recent` −0.0916 · `rls`
   −0.1604 · `gru_big` −0.1605 · `ridge_batch` −0.2119 · `mlp` −0.2419 · `gru` −0.2969. Even a 133 kB
   store fires (−2.18 %), and the parity reproduces outside the streaming harness entirely: on a single
   70/30 chronological static holdout, k-NN (k = 25) at 265.47 edges tuned GBDT at 266.33. ⚠ The tuned
   GBDT's own measured state is 3,618,071 B = **5.44×** the exemplar budget it loses to; ⚠ the strong
   baselines are single-seeded and the exemplar arms deterministic — no variance estimate is claimed,
   and none is needed at 4–25 % margins.
2. **The strong frontier's headroom over one line of code is 2.17 %.** The venue's headroom rule passes
   against persistence (574.135 MAE, headroom 41.6 %) — but the non-degenerate trivial rule is the
   weekly seasonal-naive `ŷ_j = y_{j−168}` at **342.6513**, and the best strong baseline beats *it* by
   only 2.17 % relative MAE. Both readings are in the gate artifact and neither is hidden.
3. **Destroying the stream's temporal order helps the store.** A fixed-seed uniform permutation of the
   pair sequence (`P(X, y)` and `P(y|X)` preserved exactly; positive control exact — persistence
   574.1346849611 ordered vs shuffled, diff 0) *improves* the exemplar store at and above the audited
   budgets: L = 5,037 **320.982 → 311.748** (−2.88 %); L = 14,894 (the byte budget a store build on this
   rig would occupy; **registered k = 5**) **306.762 → 301.068** (−1.86 %); L = 34,847 304.016 → 297.390
   (−2.18 %). It hurts only *below* the budget (L = 1,007 +4.71 %, L = 250 +2.69 %; crossover at
   L ≈ 2,000–5,000). ⇒ *at the byte budgets this program would use, Metro's temporal ordering carries no
   exploitable information — there is nothing there for a memory to be good at.* ⚠ The shuffle is a
   shuffle, not a drift-free data source (Appendix A.6).

  **Mechanism — the exact inverse of INSECTS':** the exemplar ladder is monotone *improving* across its
whole range (MAE **407.91 / 347.89 / 327.79 / 325.22 / 320.98 / 307.08 / 306.76 / 304.02** at
L = 250 / 500 / 1,000 / 2,000 / 5,037 / 10,000 / 14,894 / 34,847), because the regime clock
(hour-of-day, day-type, week phase) is fully encoded inside each pair's own 24-lag feature window, so a
stored exemplar never goes stale. Also inverted: raw beats causally-standardised everywhere here (320.98
vs 342.75 at L = 5,037) — the anti-hobbling max is doing its job in both directions across the two
venues. A drift map built for the venue (drift magnitude in the sense of Webb
et al. (2016, Eq. 6), with the distribution distance instantiated as total variation — Webb et al.
deliberately leave that function unspecified and use Hellinger distance in their own case study; total
variation satisfies the metric properties they give as their reason for that choice; 1,101 revisit rows;
the purely data-driven day map recovers the weekday/weekend split with no calendar input) confirms the
firing is not a favourable slice: the exemplar-vs-strong margin is negative
in **every band of every map** (−4.7 % … −17.9 %).
  **Caveats carried:** the 32-feature vector (132 B/exemplar, identical for every arm) is the analyst's
declared choice and the k-NN attack is defined by it — mitigated by giving the strong baselines exactly
the same features and by the static-holdout reproduction; feature gap-fill ≤ 3 h, targets never imputed;
a 7,386-hour sensor hole makes the stream two eras; our `knnsam` regression adaptation is ours, not a
published algorithm, and it loses to the plain window anyway (325.71 vs 320.98).

**What follows, and what does not.** ⚠ **INSECTS remains fully admissible as a *mechanics* substrate —
that half is a registered ruling of ours** (the rule being that criterion 4 gates the *value* venue, not a
mechanics build) ⟦N294 disposition⟧. **We make the same reading of Metro, and that half is our own
inference rather than a registered ruling**: nothing measured here disqualifies it as a mechanics
substrate, and nothing here certifies it as one either. What neither venue can host at these budgets is a
value claim that any memory's dynamics beat its stored content —
which is why every real-data value leg of this program is a **declared NOT-RUN for want of an admissible
venue, never a null** (Appendix J). Within this program's records these are the fifth and sixth firings
of the same admissibility rule across designed and real venues ⟦N294 / N295 dispositions⟧. The framing
is the ruled one: a fallback being retired, not a venue crisis. And the standing practice the two
firings earn is the cheap one: **run the venue tripwire first** — discovering inadmissibility after a
build is the expensive ordering, and it is free to avoid. (⚠ A proposed companion practice — publish the
20-second order-destroying shuffle on every candidate value venue before any build — is on our records
as a recommendation, not an adopted rule.)

### R.3 The 24-hour label embargo — a benchmarking-methodology finding ⟦registry: N296⟧

**The finding, in its approved form:** plain prequential evaluation at a 24 h horizon leaks up to 23 h
of future traffic to any continuously-updated learner (**+10.9 %** to a 250-exemplar k-NN, **−0.3 %** to
GBDT — asymmetric **in the direction of firing**); and seasonal-naive(`t − 24 h`) is **degenerate** at a
24 h horizon (the non-degenerate naive is `t − 168 h`).

**The mechanism, stated exactly.** Our own pre-registration asserted the ordinary test-then-train
protocol was leak-free at a 24-hour horizon, *"because pair `j`'s features stop at `j − 24` and its
label is revealed only after prediction."* That assertion is wrong by mechanism: pair `t − 1`'s **label**
is the traffic volume at target time `j − 1` — **23 hours after pair `t`'s forecast origin `j − 24`** —
so plain test-then-train hands a continuously-updated learner up to 23 hours of future labels. And the
leak is asymmetric: a k-NN store that admits every new pair immediately gains from it; a GBDT refit
every 720 pairs essentially does not.

**The fix, implemented and used for every primary Metro number:** `A(t)` = the index of the last pair
whose *target* time ≤ pair `t`'s *origin* time (`searchsorted(tgt, tgt − 24, 'right') − 1`; median
`t − A(t)` = 24, mean 22.86, max 24); every store admission, fit and online update is restricted to
indices ≤ `A(t)`.

**Measured deltas (leaky → embargoed, from the 45-arm delta table).** ⚠ **Convention, stated so it need
not be reverse-engineered:** each percentage is `(embargoed − leaky)/embargoed`, i.e. it is taken
**relative to the embargoed (conservative) value**, so a **positive** number means plain prequential
evaluation flattered that arm by that much and the embargo took it back. k-NN L = 250 std **398.17 → 446.64
= +10.85 %** · k-NN L = 250 raw 373.00 → 407.90 = +8.56 % · k-NN L = 1,007 std 344.28 → 363.59 =
+5.31 % · k-NN L = 5,037 raw 312.06 → 320.98 = +2.78 % · k-NN L = 14,894 raw 300.42 → 306.76 = +2.07 % ·
**`gbdt` 339.80 → 338.65 = −0.34 %** · `mlp` 420.27 → 414.97 = −1.28 %. ⭐ The venue verdict is
unchanged by the fix (within-protocol margin −0.0946 leaky, **−0.0615** embargoed) — i.e. the analyst
closed a leak that was biased **toward firing the analyst's own tripwire**, and R.2.2's numbers are the
embargoed, conservative ones.

**The companion defect in the same registration:** seasonal-naive(`t − 24 h`) is degenerate at a 24-hour
horizon — bit-identical to persistence, measured max |diff| = 0.0 (predicted in advance and confirmed
exactly). The non-degenerate trivial rule is the weekly `t − 168 h` naive (MAE 342.6513), and it is the
one that decides R.2.2's second check (the 2.17 % headroom).

**Scope, stated verbatim from the registration:** measured on one stream (Metro, 34,848 hourly pairs,
h = 24) with one feature construction; **the mechanism is protocol-general** for any h-step-ahead stream
evaluated test-then-train with a continuously-updated learner, **the magnitudes are not**. Two further
harness defects were declared beside it and are carried in Appendix A.6 and Appendix J (the ±10 SD
standardisation clip; the diverging `rls_ff` arm, excluded and declared NOT-RUN).

**Why it is in this paper.** It is a third measured finding about evaluation protocol in the same
species as §2.6's two (contribution 1): a control everyone runs — plain prequential scoring — is not
neutral, and its bias points in the flattering direction for exactly the memory-like arm. Anyone
auditing a bounded-state memory on an h-step-ahead stream should embargo labels to the forecast origin,
and should check their seasonal-naive is not the persistence baseline in disguise.
...
4. ✅ **The Figure 1 re-render is DONE and the outstanding-re-render disclaimer is deleted.** Figure 1 now
   carries **seven bars** (six rival arms + the CLU) at a uniform `n = 9` with NOT-RESCUED /
   INIT-UNSTABLE / SELECTION-DEPENDENT hatching exactly as Appendix K specifies, rendered from the same
   banked artifacts as §4.1.1 — ⛔ **no measurement was made or re-made for it** — with its own 14-entry
   figure → artifact → field → value provenance record beside the earlier 48-entry table (which continues
   to cover Figures 2–5, unchanged). ⚠ **Still open:** Figures 3 and 5 need their captions re-checked
   against the specs (the `d/s_fit = 3.59` audited cell; the two write loads and the 0-of-20 rival null),
   and Figure 2's target render includes the SSD bar.
...
6. ✅ **Both pending-citation items are DISCHARGED and all eight in-text `CITE2` markers are removed.**
   (a) The
   `*SEM 2018` partial-input baseline result (§1, §2.4, §5.3) is verified against the published text —
   anthology **S18-2023**, pp. **180–191**, DOI **10.18653/v1/S18-2023**; the "six of ten" figure is
   correct and the sentence is identical in preprint and proceedings, so it may be quoted without a
   version caveat. The paper's verb is *"significantly outperforms"*, and this draft now uses it. ⛔ Never
   take the venue from Semantic Scholar for this entry: it mislabels **\*SEM** as *SemEval*; the ACL
   Anthology record governs. (b) The SSD arm's citation facts are verified — **Dao & Gu, ICML 2024, PMLR
   235:10041–10071** — with the state convention triple-sourced to the official implementation and the
   authors' companion post (Appendix A.1f). ⚠ **One residual, disclosed in A.1f and not hidden:** the
   Mamba-2 paper's own body text was never read in the verification pass; the convention rests on code
   plus the authors' description.
   ⭐ **Also closed in this revision:** the Gated DeltaNet quotation restriction (the `S_t = α_t S_{t−1}
   + v_t k_tᵀ` presentation is verified in the ICLR 2025 camera-ready and may be quoted, with its authors'
   *"up to specific parameterization"* hedge attached — §5.1), and a **live mis-attribution in §5.3**: the
   *"30–80× larger"* / *"4 orders-of-magnitude"* quotes are **Chesetti & Pandey (ACDA 2025;
   arXiv:2407.00590v2 §6.6)**, not SOSD — SOSD's own size verdict runs the other way and is now printed
   beside them.
6b. ⚠ **Residual single-sourced or unverified citations, named rather than left implicit.** The following
   are cited qualitatively and have **not** been verified against published text in any pass, and **no
   number is quoted from any of them**: Wang/Shi/Fox (arXiv:2501.12352), ATLAS (arXiv:2505.23735), the
   Miras line (arXiv:2504.13173), HOLA (arXiv:2607.02303), Based, MAD, Zoology, RULER, Sparse Delta
   Memory, Erase-then-Delta, the LLM-agent-memory trivial control (arXiv:2607.21962), kNN-LM, Xu/Alon/
   Neubig, MassiveDS, Sun et al. (TTT), DeltaNet/GDN/GDN-2 beyond the loci verified above. Also
   single-sourced: **Marcus et al.'s PVLDB body text** (⛔ quote no number from it), the **ACDA
   camera-ready** of Chesetti & Pandey (quotes pinned to arXiv v2), and Mitzenmacher's **pp. 462–471**
   (two registries with shared provenance; NeurIPS's own page prints no pagination).
...
8. **Glyph strip and register pass** (⭐⛔⚠✅◐ and any remaining internal-note voice) is deferred to the
   typesetting pass. ⚠ **Named sites rather than a generic sweep**, so none is lost: the §4.1 NOT-RUN
   table note carried a never-quote instruction addressed to us rather than to the reader (*"never 'a
   preprint'"*) — **fixed in this revision**; the remaining register risks are the ⟦N…⟧ markers (item 9),
   the ⛔/⭐/⚠ glyphs everywhere, and the "we/our own records" formulations in Appendix R, which are
   correct in substance and want a copy-editing pass for a reader who has no access to those records.
9. ⚠ **Registry markers `⟦N…⟧` (§2.5, §4.8, Appendices A.5–A.6, J, L, R) are internal provenance and
   are stripped before circulation**, exactly like the earlier `⟦F3⟧` / `⟦CITE2⟧` generations. The r6
   fold's sources: N276 (the third-substrate row), N294 / N295 (the two venue-admissibility firings),
   N296 (the label embargo), N290 (the width-flag instrument label), N300 (the `out-of-control`
   NOT-RUN).
10. ✅ **DISCHARGED — the six citations introduced by the previous revision are verified and are now in
   Appendix Q.** All six were checked against primary sources (publisher record, arXiv PDF, author
   preprint, official documentation plus source tree, and the repository page with a DOI-resolution test);
   **six of six verified, none could-not-verify**, and every anchor number this paper reproduces was
   confirmed **exact** against the source: No-Change **40.46** and ARF **77.13** (the benchmark's Table 5,
   `Inc-reoc (bal.)` row), the SAM-kNN Weather row **21.74 / 21.53** (their Table IV, and they are
   *errors*, not accuracies), the window-1000 prequential convention, the 79,986-instance stream length
   and the 20 °C temperature sweep. Q.1 now holds **thirteen sources / fourteen records**. ⚠ **Three usage
   qualifiers the pass produced are applied in this revision and are listed so a reviewer can find them:**
   (a) the **Webb** attribution is split — the drift-magnitude *framework* is theirs, the
   **total-variation** instantiation is **ours** (they deliberately leave the distance unspecified and use
   Hellinger in their own case study) — §R.2.2; (b) the ARF anchor is labelled **MOA vs `river`** with the
   ensemble size marked as our inference — §R.2.1, A.6; (c) the SAM-kNN **3,000-exemplar STM cap** is
   relabelled a reference-implementation default (`ltm_size = 0.4`) rather than a published one — §R.2.1.
   ⚠ **Two residuals are declared rather than closed** (Q.2): the ARF paper's **body text was not read**
   (a 2019 correction to it exists and is unread — inert only while we quote no ARF number from it, which
   we do not), and publisher-side page images for the two DMKD papers were **SSO-blocked**, so their
   pagination rests on two agreeing independent records rather than on the publisher's own page.
11. **Appendix R adds no figure**, and the folded evidence of Appendix R is quoted in no figure: the
   figure specifications (Appendix K) are unchanged by it. ⚠ The one figure change at this revision is
   Figure 1's re-render (item 4), which draws no folded number — it is the audit's own seven bars at a
   uniform `n = 9`.
