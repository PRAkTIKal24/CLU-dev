...
table (Appendix A.5–A.6): **(i)** the audit's central statistical statement — a store whose reads are
statistically indistinguishable from a small table's — reproduces at a **third substrate**, a
Split-CIFAR-10 stream-census rig at address dimension `d = 12` (3 seeds × 3 encoder arms, pre-registered,
nothing tuned; §4.8, Appendix R.1); **(ii)** the venue-admissibility discipline of §2.5, run against the
...
designed families above did, and both venues remain perfectly good *mechanics* substrates; what neither
can host at these byte budgets is a claim that a memory's dynamics beat its own stored content. It is
...
## 4.8 The same indistinguishability at a third substrate (folded evidence)
...
The audit's central statistical statement — a store whose reads are statistically indistinguishable from
a small table's — has now been measured at a third substrate, unrelated to both of this paper's designed
families: a Split-CIFAR-10 stream-census rig at address dimension `d = 12` (a 32,768-atom store of the
...
At a third substrate, under a rule registered before the first cell, with nothing tuned, the store and a
...
### Q.1 The seven verified entries
...
before the run (σ_q/spacing 0.334 → 0.210 at `d = 12`, 3/3 seeds), and the instrument's validation was
verified in advance. The table itself is audited **in code**: it re-derives its keys from the projected
...
**77.13**. The best registered exemplar arm — SAM-kNN at its own published 0.634 MiB budget
...
within **0.07 points** of the published state-of-the-art ensemble. ⚠ The honest sentence — the spoke's
own — is ***"the byte-matched exemplar store is at ARF's shoulder"*, never "it beats ARF."* ⚠ And ARF's
...
itself (its short-term memory averages 945 of a permitted 3,000 exemplars; its long-term memory is
selected on 17.7 % of instances), and the strong reference is a recency mechanism here too (78 of ARF's
...
venues. A drift map built for the venue (total-variation drift magnitude in the sense of Webb et al.,
2016; 1,101 revisit rows; the purely data-driven day map recovers the weekday/weekend split with no
calendar input) confirms the firing is not a favourable slice: the exemplar-vs-strong margin is negative
...
**What follows, and what does not.** Both venues remain fully admissible as *mechanics* substrates; what
neither can host at these budgets is a value claim that any memory's dynamics beat its stored content —
...
**Measured deltas (leaky → embargoed, from the 45-arm delta table):** k-NN L = 250 std **398.17 → 446.64
...
9. ⚠ **Registry markers `⟦N…⟧` (§2.5, §4.8, Appendices A.5–A.6, J, L, R) are internal provenance and
   ...
   NOT-RUN).
10. ⚠ **New citations introduced by the r6 fold, unverified in any pass of ours and named rather than
   left implicit** (same discipline as item 6b): the INSECTS benchmark authors (Souza et al. — year and
   venue not verified), SAM-kNN (Losing et al.), adaptive random forests (ARF; Gomes et al.), the
   `river` streaming library, the Metro Interstate Traffic Volume dataset (UCI Machine Learning
   Repository), and Webb et al. (2016) for drift magnitude. Every number quoted near them traces to our
   own runs (Appendix A.6), except the published anchors those runs reproduce (No-Change 40.46, ARF
   77.13, the SAM-kNN Weather row), which are quoted as reproduction targets only. ⛔ None of these is
   in Appendix Q; a cite-check pass is owed before circulation.
11. **Appendix R adds no figure**; the figure specifications (Appendix K) are unchanged by the r6 fold,
   and the Figure 1 re-render item (item 4) is likewise unchanged.
