...
(**−0.3180 ± 0.0804**). The rescue gate that disqualifies two rival arms disqualifies ours as well: our
store's written read sits **below** its own blank-store control (**−0.5261 ± 0.0863** against **−0.4221**,
three seeds), which we report as a finding rather than a footnote. **(ii)** The control the field would
...
formality: on the dividend family it disqualifies one of five rival arms outright, leaves a second
unresolved, **and disqualifies our own store, whose written read sits below its own blank-store control**
(§4.1.1); and on the byte-frontier column **none of the fifteen (arm × head-width) cells measured at nine
seeds clears it** (§4.5).
...
- **Coverage, printed with the verdict, including the part that is uncomfortable.** Our store-side studies
  ...
  *write-admissible*. By that criterion **`aggregate` is 0 of 3 and `manifold` is 0 of 3** — endpoint
  ...
  mechanism for the below-blank read we report for ourselves in §4.1.1 and is a fact about our store
...
**Seed counts, stated once and never mixed silently:** on the dividend family **every rival column is at a
uniform `n = 9`** (seeds 0–8) under the full tuning grid — `full`, the projected launder and the dividend
against it, the same-keys null, the blank-store control, both `+0 B` margins and the rescue lift, with the
byte ledger reported per seed where it varies (Appendix I.1c); the CLU column is **n = 3** (seeds 0, 1, 2,
banked and reproduced digit-for-digit rather than re-measured); on the byte-frontier column three of the
five arms were re-measured at **n = 9** on the current code path and two were not (§4.5, Appendix H,
Appendix J). Flag-provenance tables, one per run: Appendix A.
...
quoted at **n = 9** for every rival arm, and so now is every other rival column: the projected launder, the
dividend against it, the same-keys null and the blank-store control were re-aggregated to the same nine
seeds from the same runs (Appendix I.1c). The byte-ledger columns are exact integers, but they are
**not seed-independent for the TTT arms** — best-of-grid selects the mini-batch `b` per seed and `b` sits
inside the declared state, so those rows carry a per-seed ledger and a `b → (d_head, state, table)` mapping
...
|---|---|---|---|---|---|---|---|---|---|---|
| ttt_linear | 29 / 36 (per seed) | 5592 / 7944 | 5220 / 5328 | 5104 / 5184 | −0.6075 ± 0.1096 | **−0.2213 ± 0.1062** | **−0.4602 ± 0.1038** | +0.093 ± 0.134 | +0.320 ± 0.083 | ⚠ **UNSTABLE** — clears under one path, not the other |
| ttt_mlp | 12 | 5736 | 4656 / 5376 | 4608 / 5376 | −0.5898 ± 0.0731 | **−0.2095 ± 0.0683** | **−0.4425 ± 0.0869** | −0.071 ± 0.090 | +0.093 ± 0.107 | ⛔ **NOT RESCUED** (no configuration) |
...
| **CLU** (`n = 3`) | — | 5376 | 5200 | 100 | **−0.5261 ± 0.0863** | **−0.3180 ± 0.0804** | (its own table is already raw) | **−0.104** (against blank **−0.4221**) | — | ⛔ **NOT RESCUED** — the written store reads **below** its own blank store |

⭐ **Our own arm fails our own gate, and that is a result, not an embarrassment.** The CLU's full read on
the audited cell is **−0.5261 ± 0.0863** while the same read path against a store with nothing written in
it is **−0.4221**: the lift is **−0.104**, i.e. negative, so the arm cannot clear a gate that requires the
lift to exceed **+2 SE** of itself, under any value that SE could take. (The banked artifact records the
blank control as a three-seed mean without a paired per-seed spread, so we quote no SE on the lift; the
sign settles the verdict on its own.) The honest reading is the one this paper's thesis already predicts:
on this family the *written content does not lift the read*, and a store whose content does not lift its
...
write budget). The same
pattern was measured once before on a family our own protocol validation struck (`recency`: 0.4769 written
against 0.5463 blank, §2.5, Appendix L). ⛔ Consequently **no comparative margin in favour of the CLU over
any rival arm is quotable anywhere in this paper**, and none is drawn.

⚠ **And the gate's power caveat cuts our way too.** §2.2's finding — that this gate is not interpretable
below nine seeds when its control is a single initialisation draw — applies to our own column as much as
to the rivals'. What we report is therefore a **three-seed sign fact** (the point lift is negative, so no
value of its standard error could rescue the arm *at these three seeds*), not a nine-seed verdict of the
kind we quote for the rival arms. Whether that sign holds at nine seeds is untested and is declared a
NOT-RUN (Appendix J); we do not present our own failure as more robust than the passes we refused to
quote.
...
already a raw table of `(key, payload)` rows — and its dividend over that launder is
**−0.0789 ± 0.0620** (three seeds; per-seed −0.1863 / +0.0284 / −0.0788), i.e. the launder reads better
than the store does.

**Rescue-gate verdicts, at nine seeds (rivals) and three (ours).** **RESCUED: `deltanet`, `gdn`, `gdn2`** — under both code paths.
⛔ **NOT RESCUED: `ttt_mlp`**, in every configuration we ran, so **no comparative margin in favour of any
other arm over TTT-MLP is quotable**
...
readings, treat the verdict as unresolved, and **quote no comparative margin over TTT-Linear** either. ⛔ **NOT
RESCUED: the CLU**, on its own three seeds (above). Only the three rescued rival arms enter the falsifier
adjudications of §4.3 as arms we compare *against*.

⭐ **A rival-side observation the uniform re-aggregation makes visible, and it splits the two families.**
Against the **same-keys null** — the identical read path against a store written with the same keys and a
permuted payload assignment — the paired difference `full − null` at nine seeds is
...
+0.6832). This is a statement about the rival arms and it changes no claim of ours; we report it because a
reader of the audit table is entitled to know which arms are reading their own payloads at all.

⚠ **The TTT rows' byte ledger is a per-seed quantity, and no single TTT byte figure is *the* nine-seed
value.** Best-of-grid selects the mini-batch `b` per seed, and `b` sits inside the declared state (it is
the in-flight buffer), so the head width and every byte column move with it:
...
`b`-dependent for the TTT arms by construction (Appendix B.2, Appendix O). The delta-rule rows are
...
store on seeds 0–7 and **6 of 8** on seed 8. The drops are the family's own construction rule (a query
whose target lands within payload tolerance of a stored payload is dropped) — which is exactly what stops
the arg-min launder from being accidentally right — and the coverage range is wider across the six added
seeds than across the registered three, which a reader should weigh against every per-seed quantity here.
...
seeds (−0.2592 … −0.4602). ⚠ **The two stress columns are less powered than the result they defend** —
they are three-seed re-selections from the same fits, they are labelled as such at every appearance
(Appendix I.1d), and they agree with the nine-seed primary in sign on 5 of 5 arms and in direction on
every column.

⚠ **On optional stopping, since seeds 3–8 were added after seeds 0–2 had been seen.** The addition was
...
⚠ **And the honest qualifier on the arm count — which applies to our own arm too.** Two of the five rival
arms do not settle the rescue gate at nine seeds — TTT-MLP clears its own blank-store control in no
configuration we ran, and TTT-Linear clears it under one initialisation scheme and not the other (§4.1.1)
— so for those two the statement *"it loses to a raw table at its own bytes"* is partly a statement about
an arm that may not be reading its store at all. ⛔ **The identical qualifier attaches to the CLU**, which
fails the same gate outright on its three seeds (full −0.5261 ± 0.0863 against a blank of −0.4221): our
own *"it loses to a raw table at its own bytes"* is likewise partly a statement about an arm whose written
content is not lifting its read. We apply the qualifier to ourselves in the same paragraph in which we
apply it to the rivals, because a gate that disqualifies only other people's arms is not a gate. **On the
...
- The CLU's dividend over its own matched-byte launder is **−0.0789 ± 0.0620** (three seeds; per-seed
  −0.1863 / +0.0284 / −0.0788) — i.e. the launder is *better*. At
...
- **Weak form, measured at nine seeds.** Against the **arg-min** control read through each memory's own
  ...
  **DeltaNet +0.1515 ± 0.0600 · GDN +0.5960 ± 0.0933 · GDN-2 +0.6824 ± 0.0756**, all three positive by
  more than 2 SE, while the two TTT arms are negative (**−0.1840 ± 0.1069** and **−0.1794 ± 0.0748**) and
  **the CLU is −0.0789 ± 0.0620** (three seeds). In that reading, **test-time dynamics pays for the
  delta-rule family and does not pay for ours.** That sentence is true as measured, it is in this paper,
...
invariant this table is actually about (Appendix B.2, Appendix I.1c(e)).
...
**Rivals measured beside it — and the column is a LABELLED NULL, at nine seeds.** Three arms
(`deltanet`, `ttt_linear`, `gdn2`) were measured on this column at **nine seeds** on the current code
path, at five head widths each. ⛔ **Not one of the fifteen (arm × head-width) cells clears the rescue
gate**: every cell sits within 2 SE of its own blank store, and the largest lift anywhere is
**+0.0694 ± 0.0491** (`deltanet` at `d_head = 4`). The reason is a property of the venue rather than of
...
⚠ `deltanet`'s rescue at nine seeds is an `aggregate` verdict and **does not transfer here**: an arm that
functions on one family is not thereby functioning on another. ⚠ Two arms (`ttt_mlp`, `gdn`) were never
run on this column and are declared NOT-RUN rather than reported as nulls (Appendix J). ⚠ And a byte-ledger
...
> achieves (**4.3 well-widths**, atom-width ruler; **3.72** on the fitted-width ruler) the coupling is
...
column none of the fifteen cells measured at nine seeds separates from its control; ⛔ **and our own store
fails the same gate on the dividend family — its written read (−0.5261 ± 0.0863) sits below its own blank
store (−0.4221)**. **No comparative margin in favour of any arm over TTT-MLP, TTT-Linear or the CLU
appears anywhere in this paper**, so the cross-family audit is carried by three arms
...
than count to five. ⚠ The gate's direction rule (Appendix B.5) is what keeps this from being vacuous: it
suppresses *comparative* margins in the flattering direction and never an arm's own loss to its own
byte-matched table — which is the only quantity the headline uses.
...
float32, on CPU — nine seeds on the rival dividend arms and on the three rival arms of the byte-frontier
column, three on the CLU column, the protocol-validation run, the first pass's frontier rows and the
attribution sweep. **Nothing here transfers to a language-model claim**,
...
under a held-out selection rule), and not ours (−0.3180 ± 0.0804 over three seeds) — an arm which, on the
same three seeds, does not clear the audit's own functioning check either, reading below its own
blank-store control. Against a weaker but
...
⚠ **The gate applies to us on the same terms.** On the dividend family our own store's full read
(−0.5261 ± 0.0863) sits **below** its blank-store control (−0.4221), so the lift is negative, the CLU is
**NOT RESCUED**, and no comparative margin in our favour is quotable anywhere in this paper (§4.1.1).
...
under one initialisation scheme, not the other); ⛔ **NOT RESCUED** the **CLU** (n = 3, below its own
blank store). On the byte-frontier column **none of the fifteen (arm × head-width) cells measured at nine
seeds** separates from its control — so that column carries **no quotable margin in either direction**
(§4.5), and two of the five arms were never run there at all.
...
**The reading, and it is the whole reading.** ⛔ **0 of 15 cells are rescued at nine seeds.** The best lift
anywhere is `deltanet@d_head = 4` at **+0.0694 ± 0.0491**, which does not clear 2 SE. Every arm lives in
**0.12–0.24** on a six-way choice whose chance level is 0.167: the column has no resolving power at this
budget, which is a property of the venue rather than of the rivals. ⚠ `deltanet`'s rescue on `aggregate`
does **not** transfer here. ⚠ At `d_head ≤ 8` the delta-rule cells are **not table-lossless** (the
...
| **CLU** (banked, reused not re-measured) | — | 57384 | — | — | **0.9722 ± 0.0139** | **1.0000** | 0.1667 |
...
this work.** Every rival column below is at `n = 9` (seeds 0–8), pooled by the same aggregation routine
that produced the published table, from the same runs (Appendix A.1c) — nothing was re-measured. The CLU
column is banked at `n = 3` and is never re-derived; the two `n`s are labelled and never pooled.
...
| deltanet | −0.4205 ± 0.0299 | −0.5720 ± 0.0653 | **+0.1515 ± 0.0600** | −0.6379 ± 0.0708 | −0.7147 ± 0.0800 | +0.2174 ± 0.0749 | +0.2943 ± 0.0766 | ✅ |
| gdn | −0.4073 ± 0.0120 | −1.0033 ± 0.0952 | **+0.5960 ± 0.0933** | −0.9715 ± 0.0982 | −1.2869 ± 0.2317 | +0.5642 ± 0.1032 | +0.8796 ± 0.2273 | ✅ |
| gdn2 | −0.4065 ± 0.0178 | −1.0889 ± 0.0815 | **+0.6824 ± 0.0756** | −1.1503 ± 0.1165 | −1.4319 ± 0.3241 | +0.7438 ± 0.1242 | +1.0254 ± 0.3293 | ✅ |
| **CLU** (banked, `n = 3`) | **−0.5261 ± 0.0863** | **−0.4472** | **−0.0789 ± 0.0620** | **−0.8175** | **−0.4221** | — | **−0.104** (point) | ⛔ **NOT RESCUED** |
...
| **CLU** (banked, `n = 3`, **not** superseded — this is the CLU column of record) | **−0.4472** | **−0.0789 ± 0.0620** | **−0.4221** | **−0.8175** |

**What the uniform-`n` table changes, and what it does not.** The headline is unchanged and is now
uniform-`n` on every column. The rescue verdicts are unchanged. The dividends against the projected
...
- **Two of the five arms on the byte-frontier column: `ttt_mlp` and `gdn`.** The column was re-measured at
  nine seeds for `deltanet`, `ttt_linear` and `gdn2` (Appendix H.1); the other two arms have no row on it
  and are not reported as nulls. ⚠ Since 0 of the 15 measured cells clears the rescue gate, running them
  would extend a labelled null rather than change a verdict — but it would be needed before any sentence
  quantified over "all five arms" on that column, and this paper writes no such sentence.
- **The byte-frontier column under the full tuning grid.** H.1's rows are at nine seeds on the current
  ...
  effects.
- **The CLU column at nine seeds.** Our own column is the banked three-seed one, reproduced
  digit-for-digit rather than re-measured; the rival columns are at nine. The asymmetry is visible, it is
  labelled at every appearance, and it is not closed by anything in this paper: the CLU's `+0 B` margin is
  about four SE below zero *on its own three seeds*, and its failure of the rescue gate is a point
  statement without a paired spread (§4.1.1). Re-running that column at nine seeds is the cheapest single
  strengthening available to this work.
...
drawn, every arm that fails the rescue gate hatched and labelled NOT RESCUED — **which includes the CLU
bar, and the caption must say so in the same breath as the rival hatching** — the arm whose verdict is
unstable hatched differently and labelled UNSTABLE, and the CLU bar in a distinct fill so that the
authors' own arm is identifiable. Error bars = ±1 SE, **n = 9 on the rival bars and n = 3 on the CLU bar —
the caption must say so**, along with family, byte budget, tuning grid, and the scale qualifier. Not
rendered in this draft.
...
falls 478× → 2.28×), with the rival points **omitted** and a caption stating why: at nine seeds **0 of 15
(arm × head-width) cells on that column clear their own blank-store control**, so the rival points would
be a picture of noise, and two of the five arms were never run there at all (Appendix H.1, Appendix J).
...
1. **The audit's own headline is negative for our system**: dividend −0.0789 ± 0.0620 against its own
   launder, +0 B margin −0.3180 ± 0.0804, and an earlier round of the same audit went 0-for-4.
1a. ⛔ **Our store fails our own rescue gate on the audited family** (§4.1.1): full read
   −0.5261 ± 0.0863 against a blank store of −0.4221, a lift of **−0.104**. The written content does not
   lift the read, so no comparative margin in our favour is quotable anywhere in this paper. This is the
...
Seed counts are stated per section and never mixed silently: the rival dividend arms are 9-seeded on
**every** column (Appendix I.1c), three of the five arms on the byte-frontier column are 9-seeded
(Appendix H.1) and two were not run there; the CLU column, the protocol-validation run, the first pass's
frontier rows and the attribution sweep are 3-seeded (Appendix A). Statistics conventions are stated per section. ⭐ **The audit's first pass reproduces
...
reader at **−0.2493**; saturation score **0.5068**.
...
not have), and Mamba-2 / GRU / sliding-window attention (outside the ruled arm set for this pass). All are
listed in Appendix J with their reasons, and §6 L5 never blurs measured against reasoned.
...
2000-step re-check on the sub-grid containing every 400-step winner, nine seeds. The outer parameters
...
asserted as an integer identity per cell (Appendix B.7). ⛔ That ratio is unreachable-by-construction, not
a budget choice (§3.1).
...
1. **Number-freeze gate is CLOSED, and the rival columns are now uniform-`n`.** Every rival number in this
   draft is the full-grid, nine-seed value on **every** column; the first pass survives only as labelled
   appendix material (A.1b, H.2, I.1–I.1d).
...
4. **Figures are specified, not rendered** (Appendix K). Figure 1 is renderable from the frozen artifacts;
   its caption must now also carry the CLU's NOT-RESCUED status, not only the rival hatching.
...
6. ⚠ **One remaining single-sourced citation to verify before printing:** the `*SEM 2018` partial-input
   baseline result cited in §5.3 (anthology ID, pages, and the "6 of 10" figure) has not been re-verified
   against the published text in this pass.
7. ⚠ **The visible `n`-asymmetry is unclosed by design of scope:** rivals at nine seeds, our own column at
   three. It is labelled everywhere and declared in Appendix J; re-running our column at nine seeds is a
   funding decision, not a drafting one.
